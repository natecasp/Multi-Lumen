...
function checkCrescentCornersAboveCircularMidline(crescentSystem) {
    if (!(crescentSystem instanceof CrescentSystem)) {
        console.warn('This function requires a CrescentSystem instance');
        return false;
    }
    
    // Get the current geometry configuration
    const geometry = crescentSystem.calculateIndependentCrescentGeometry();
    
    // Determine which circular center Y to use based on mode
    const circularCenterY = geometry.isIndependentMode ? 
        geometry.actualCircularCenterY : 
        geometry.circularCenterY;
    
    // Calculate the Y coordinate of the circular lumen's horizontal midline
    const circularMidlineY = circularCenterY;
    
    // Get the crescent geometry to find the actual corner positions
    const crescentGeometry = crescentSystem.evenWallThickness ? 
        crescentSystem.calculateCrescentGeometry() : 
        crescentSystem.calculateCrescentGeometryWithDiameter(crescentSystem.phantomCircularDiameter);
    
    // The actual corner Y positions are at the corner centers
    // Both left and right corners should be at the same Y level
    const leftCornerY = crescentGeometry.leftCornerCenterY;
    const rightCornerY = crescentGeometry.rightCornerCenterY;
    
    // Use the corner center Y position (they should be the same)
    const cornerY = leftCornerY + Number($("#crescent-corner-radius").val()); // or rightCornerY, they should be equal
    
    // Check if corners are at or above the circular midline
    const cornersAboveMidline = cornerY >= circularMidlineY;
    
    // Optional: Add detailed logging
    const unit = $("#unit").val() || 'in';
    console.log(`🔍 Crescent Corner Analysis:`);
    console.log(`   Circular center Y: ${circularCenterY.toFixed(4)}${unit}`);
    console.log(`   Circular midline Y: ${circularMidlineY.toFixed(4)}${unit}`);
    console.log(`   Left corner center Y: ${leftCornerY.toFixed(4)}${unit}`);
    console.log(`   Right corner center Y: ${rightCornerY.toFixed(4)}${unit}`);
    console.log(`   Corner Y position: ${cornerY.toFixed(4)}${unit}`);
    console.log(`   Corners above/at midline: ${cornersAboveMidline ? '✅ YES' : '❌ NO'}`);
    console.log(`   Difference: ${(cornerY - circularMidlineY).toFixed(4)}${unit}`);
    
    return cornersAboveMidline;
}
...
const BASE_VALUES_INCHES = {
    ...
    defaultCylinderRadius: 0.1,     // Default cylinder radius (0.200 diameter)
...
function updateSystemParameters(fromUnit, toUnit) {
    ...
    system.minimumSeptum = baseValues.minimumSeptum;
...
function updateCameraConstraints(unit) {
    ...
    if (globalOrthographicCamera) {
        ...
        const cameraDistance = unit === 'mm' ? 15.24 : 0.6;
...
setupGlobalDistanceListeners() {
    ...
    globalDistanceSlider.addEventListener('input', (e) => {
        startInteraction();
        autoExitPrintMode();
    ...
    globalDistanceInput.addEventListener('input', (e) => {
        autoExitPrintMode();
        const newDistance = parseFloat(e.target.value);
    ...
    if (peripheralHoles.length > 0) {
        ...
        globalDiv.innerHTML = `
            ...
            <div class="input-group">
                ...
                <input type="range" class="range-slider" id="global-distance" min="0" max="${maxGlobalDistance.toFixed(3)}" value="${globalDistance.toFixed(4)}" step="${getStepValue(unit, 'diameter')}">
                <input type="number" id="global-distance-input" min="0" max="${maxGlobalDistance.toFixed(3)}" value="${globalDistance.toFixed(4)}" step="${getStepValue(unit, 'diameter')}">
            </div>
        `;
    ...
    this.holes.forEach((hole, index) => {
        ...
        let controlsHTML = `
            ...
            <div class="input-group">
                ...
                <input class="range-slider c-${hole.color}" type="range" min="0.015" max="${maxHoleDiameter.toFixed(3)}" value="${(hole.radius * 2).toFixed(4)}" step="${getStepValue(unit, 'diameter')}" id="hole-${index}-diameter-range" >
                <input type="number" id="hole-${index}-diameter" value="${(hole.radius * 2).toFixed(4)}" min="0.015" max="${maxHoleDiameter.toFixed(3)}" step="${getStepValue(unit, 'diameter')}">
            </div>
        `;
...
createAxisLines() {
    ...
    const z = this.cylinderHeight / 2 + 0.002;
    const spacing = getDimensionSpacing(unit);
    const axisLength = spacing.axisLength;
...
getMinimumSeptumThickness() {
  const unit = $("#unit").val() || 'in';
    ...
    return outerDiameter <= getBaseValues(unit).defaultCylinderRadius*2 ? getBaseValues(unit).minimumSeptum : getBaseValues(unit).minimumSeptum * 1.6;
}
...
updateSingleHole(holeIndex) {
    ...
    // UPDATED: Only update geometry, don't call distance limits here
    this.updateCylinderGeometry();
    this.updateHoleUI(holeIndex);
    
    // Update distance limits separately to avoid circular calls
    this.updateSingleHoleDistanceLimitsOnly(holeIndex);
}
 updateSingleHoleDistanceLimitsOnly(holeIndex) {
    const hole = this.holes[holeIndex];
    
    // Use the same logic as updateSingleHoleDistanceLimits but without calling updateSingleHole
   ...
   const unit = $("#unit").val() || 'in';
    ...
    this.holes.forEach((hole, index) => {
        ...
        div.innerHTML = `
            ...
            <div class="input-group">
                <label>Distance:</label>
                <input type="range" class="range-slider c-${hole.color}" id="smile-hole-${index}-distance-range" min="${minDistance.toFixed(3)}" max="${maxDistance.toFixed(3)}" value="${useDistance.toFixed(4)}" step="${getStepValue(unit, 'diameter')}">
                <input type="number" id="smile-hole-${index}-distance" min="${minDistance.toFixed(3)}" max="${maxDistance.toFixed(3)}" value="${useDistance.toFixed(4)}" step="${getStepValue(unit, 'diameter')}">
            </div>
        `;
  ...
  const unit = $("#unit").val() || 'in';
    ...
    this.holes.forEach((hole, index) => {
        ...
        div.innerHTML = `
            ...
            <div class="input-group">
                <label>Diameter:</label>
                <input type="range" class="range-slider c-${hole.color}" id="smile-hole-${index}-diameter-range" min="0.005" max="0.06" value="${(hole.radius * 2).toFixed(4)}" step="${getStepValue(unit, 'diameter')}">
                <input type="number" id="smile-hole-${index}-diameter" min="0.005" max="0.06" value="${(hole.radius * 2).toFixed(4)}" step="${getStepValue(unit, 'diameter')}">
            </div>
            <div class="input-group">
                <label>Angle:</label>
    ...
    if (globalDistanceSlider && globalDistanceInput) {
        ...
        globalDistanceSlider.addEventListener('input', (e) => {
            ...
            if (this.currentSystem instanceof CircularHolesSystem) {
                globalDistanceInput.value = newDistance.toFixed(4);
                this.currentSystem.updateGlobalDistance(newDistance);
            }
        });
        ...
        globalDistanceInput.addEventListener('input', (e) => {
            ...
            if (!isNaN(newDistance)) {
                if (this.currentSystem instanceof CircularHolesSystem) {
                    globalDistanceSlider.value = newDistance;
                    this.currentSystem.updateGlobalDistance(newDistance);
                }
            }
        });
...
this.setupGlobalDistanceControl();
...
function checkCrescent() {
  if(window.profileManager.currentSystem instanceof CrescentSystem) {
    if(checkCrescentCornersAboveCircularMidline(window.profileManager.currentSystem)) {
      $("#crescent-height-warning").dialog("close");
      return true;
    } else {
      $("#crescent-height-warning").dialog("open");
      return false;
    }
  } else {
    return true;
...
function checkAndPromptCentralHole() {
  if(window.profileManager.currentSystem instanceof CircularHolesSystem) {
    ...
    if (needsCentralHole) {
      console.log('Central hole recommended - prompting user');
...
function checkCentralHoleCompliance() {
  if(window.profileManager.currentSystem instanceof CircularHolesSystem) {
    ...
    // If we already have a central hole, we're compliant
    ...
    // Return true if no central hole is needed, false if it's needed but missing
    return !needsCentralHole;
  } else {
...
function checkSurfaceArea() {
  if(window.profileManager.currentSystem instanceof CircularHolesSystem) {
    ...
    // Calculate combined surface areas of all holes
    ...
    // Check if ratio is between 2.5 and 10.25
    return ratio >= 2.5 && ratio <= 10.25;
  } else {
    return true; // For non-circular systems, always return true
  }
}
...
function checkHoleDistribution() {
  if(window.profileManager.currentSystem instanceof CircularHolesSystem) {
    ...
    // Calculate slice size based on number of peripheral holes
    const sliceAngle = (360 / peripheralHoles.length); // degrees per slice
    const sliceAngleRad = (2 * Math.PI) / peripheralHoles.length; // radians per slice
    
    console.log(`Checking distribution: ${peripheralHoles.length} holes, ${sliceAngle}° per slice`);
    
    // Check each slice
    for (let sliceIndex = 0; sliceIndex < peripheralHoles.length; sliceIndex++) {
      const sliceStartAngle = sliceIndex * sliceAngle; // degrees
      const sliceEndAngle = (sliceIndex + 1) * sliceAngle; // degrees
      
      let sliceHasHole = false;
      
      // Check if any hole intersects with this slice
      peripheralHoles.forEach(hole => {
        // Calculate the angular span of the hole
        const holeAngle = hole.angle; // degrees (0-360)
        const holeDistance = hole.distance;
        const holeRadius = hole.radius;
        
        // Calculate the angular width of the hole at its distance from center
        // This is the angle subtended by the hole radius at the given distance
        const holeAngularWidth = Math.asin(holeRadius / holeDistance) * (180 / Math.PI); // degrees
        
        const holeStartAngle = holeAngle - holeAngularWidth;
        const holeEndAngle = holeAngle + holeAngularWidth;
        
        // Check if the hole overlaps with the current slice
        // Handle wraparound at 360/0 degrees
        if (anglesOverlap(sliceStartAngle, sliceEndAngle, holeStartAngle, holeEndAngle)) {
          sliceHasHole = true;
        }
      });
      
      if (!sliceHasHole) {
        console.log(`Empty slice found: ${sliceStartAngle}° to ${sliceEndAngle}°`);
        highlightEmptySlice(sliceStartAngle, sliceEndAngle, system);
        return false; // Found an empty slice
      }
    }
    
    console.log('All slices have holes - distribution is good');
    return true; // All slices have holes
  } else {
    return true; // For non-circular systems, always return true
  }
}
...
function anglesOverlap(start1, end1, start2, end2) {
  ...
  if (start1 > end1) { // Range 1 wraps around 360/0
    return anglesOverlap(start1, 360, start2, end2) || anglesOverlap(0, end1, start2, end2);
  }
  
  if (start2 > end2) { // Range 2 wraps around 360/0
    return anglesOverlap(start1, end1, start2, 360) || anglesOverlap(start1, end1, 0, end2);
  }
...
function highlightEmptySlice(startAngleDeg, endAngleDeg, system) {
  ...
  const outerRadius = system.cylinderRadius * 1.1; // Slightly larger than cylinder
  const thetaLength = endAngleRad - startAngleRad;
...
function calculate() {
  if (checkMaterial() && checkOD() && checkLength() && checkQuantity() && checkCrescent() && checkSurfaceArea() && checkCentralHoleCompliance() && checkHoleDistribution()) {
