...
function createOuterDiameterDimension(system, z) {
    ...
    const extensionDistance = outerRadius + 0.08;
    
    // Create horizontal dimension line at the bottom
    const leftPoint = new THREE.Vector3(-outerRadius, 0, z);
    const rightPoint = new THREE.Vector3(outerRadius, 0, z);
    const leftExtension = new THREE.Vector3(-extensionDistance, -0.03, z);
    const rightExtension = new THREE.Vector3(extensionDistance, -0.03, z);
    
    // Main dimension line (horizontal)
    const mainLineGeometry = new THREE.BufferGeometry().setFromPoints([
        leftExtension, rightExtension
    ]);
    const mainLineMaterial = new THREE.LineBasicMaterial({ color: 0x000000, linewidth: 2 });
    const mainLine = new THREE.Line(mainLineGeometry, mainLineMaterial);
    globalScene.add(mainLine);
    system.dimensionLines.push(mainLine);
    
    // Left extension line
    const leftExtensionGeometry = new THREE.BufferGeometry().setFromPoints([
        new THREE.Vector3(-outerRadius, 0.02, z),
        new THREE.Vector3(-outerRadius, -0.05, z),
        leftExtension
    ]);
    const leftExtensionLine = new THREE.Line(leftExtensionGeometry, new THREE.LineBasicMaterial({ color: 0x666666, linewidth: 1 }));
    globalScene.add(leftExtensionLine);
    system.dimensionLines.push(leftExtensionLine);
    
    // Right extension line
    const rightExtensionGeometry = new THREE.BufferGeometry().setFromPoints([
        new THREE.Vector3(outerRadius, 0.02, z),
        new THREE.Vector3(outerRadius, -0.05, z),
        rightExtension
    ]);
    const rightExtensionLine = new THREE.Line(rightExtensionGeometry, new THREE.LineBasicMaterial({ color: 0x666666, linewidth: 1 }));
    globalScene.add(rightExtensionLine);
    system.dimensionLines.push(rightExtensionLine);
    ...
    labelDiv.style.cssText = `
        ...
        white-space: nowrap;
    `;
    labelDiv.innerHTML = `OD: ${outerDiameter}m`;
    
    const outerDimensionLabel = new CSS2DObject(labelDiv);
    outerDimensionLabel.position.set(0, -0.08, z); // Below the dimension line
    globalScene.add(outerDimensionLabel);
    system.dimensionLabels.push(outerDimensionLabel);
}
