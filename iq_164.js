...
function addCentralHoleAutomatically() {
  if(window.profileManager.currentSystem instanceof CircularHolesSystem) {
    ...
    const holeCountInput = document.getElementById('hole-count');
    if (holeCountInput) {
      const newCount = Math.min(9, system.holes.length + 1);
      holeCountInput.value = newCount;
    }
    
    // Store existing hole data before regenerating
    const existingHoleData = peripheralHoles.map(hole => ({
      angle: hole.angle,
      distance: hole.distance,
      radius: hole.radius,
      color: hole.color,
    ...
    // Regenerate the holes with the central lumen
    const currentPeripheralCount = peripheralHoles.length;
    const totalCount = Math.min(9, currentPeripheralCount + 1);
    system.regenerateHoles(totalCount);
    
    // Restore the original hole data for peripheral holes
    existingHoleData.forEach((holeData, index) => {
      // Find the corresponding hole in the regenerated system
      // Central lumen will be at index 0, so peripheral holes start at index 1
      const systemHoleIndex = index + 1;
      
      if (systemHoleIndex < system.holes.length && system.holes[systemHoleIndex].distance > 0) {
        const hole = system.holes[systemHoleIndex];
        hole.angle = holeData.angle;
        hole.distance = holeData.distance;
        hole.radius = holeData.radius;
        hole.color = holeData.color;
        hole.name = holeData.name;
        
        // Recalculate x, y coordinates based on preserved angle and distance
...
function calculate() {
  if (checkMaterial() && checkOD() && checkLength() && checkQuantity() && checkCrescent() && checkSurfaceArea() && checkCentralHoleCompliance()) {
