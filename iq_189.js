...
updateGlobalDistanceLimits() {
    ...
    // Calculate limits based on available space constraint
    ...
    const maxDistance = cylinderInnerEdge - maxHoleRadius;
    let minDistance = maxHoleRadius + spacingConstraint;
    
    // If any hole would be in the semicircle area at minimum distance, increase it
    const minYPosition = spacingConstraint + maxHoleRadius;
    const testMinDistance = Math.max(minDistance, semicircleOuterEdge + maxHoleRadius);
    
    globalDistanceSlider.min = testMinDistance.toFixed(3);
    globalDistanceSlider.max = maxDistance.toFixed(3);
    globalDistanceInput.min = testMinDistance.toFixed(3);
    globalDistanceInput.max = maxDistance.toFixed(3);
    
    // Update current value if it's outside the new range
    const currentDistance = this.holes[0]?.distance || testMinDistance;
    if (currentDistance < testMinDistance || currentDistance > maxDistance) {
        const clampedDistance = Math.max(testMinDistance, Math.min(maxDistance, currentDistance));
        globalDistanceSlider.value = clampedDistance.toFixed(4);
        globalDistanceInput.value = clampedDistance.toFixed(4);
        this.updateGlobalDistance(clampedDistance);
    } else {
...
const colors = ['Green', 'Blue', 'Red', 'Orange', 'Magenta', 'Cyan', 'Yellow', 'Purple', 'Light Green'];
  ...
  updateCylinderGeometry() {
   ...
   autoExitPrintMode();
    ...
    if (isAltPressed) {
        // Apply to all holes using the global distance method
        this.updateGlobalDistance(newDistance);
        ...
        // Apply to active hole only - existing logic
        const hole = this.holes[this.activeHoleIndex];
        const { x, y } = this.polarToCartesian(hole.angle, newDistance);
        ...
        if (this.isValidPosition(x, y, hole.radius, this.activeHoleIndex)) {
            ...
            const marker = this.holeMarkers[this.activeHoleIndex];
            if (marker) {
                marker.position.set(hole.x, hole.y, this.cylinderHeight / 2);
            }
            const control = this.polarControls[this.activeHoleIndex];
            if (control) {
                control.position.set(hole.x, hole.y, this.cylinderHeight / 2);
            }
            
            this.updateCylinderGeometry();
            this.updateHoleUI(this.activeHoleIndex);
        }
    }
        }else {
...
createSmileHoleUI() {
    ...
    if (this.holes.length > 0) {
        ...
        globalDiv.innerHTML = `
            ...
            <div class="input-group">
                <label>Distance:</label>
                <input type="range" class="range-slider updater" id="global-distance" min="${minGlobalDistance.toFixed(3)}" max="${maxGlobalDistance.toFixed(3)}" value="${globalDistance.toFixed(4)}" step="${getStepValue(unit, 'diameter')}">
                <input type="number" class="updater" id="global-distance-input" min="${minGlobalDistance.toFixed(3)}" max="${maxGlobalDistance.toFixed(3)}" value="${globalDistance.toFixed(4)}" step="${getStepValue(unit, 'diameter')}">
            </div>
        `;
        container.appendChild(globalDiv);
