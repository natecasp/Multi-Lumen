...
createHoleCenterCircle() {
    ...
    // Position the label at a 45-degree angle from center
    const labelAngle = Math.PI / 4; // 45 degrees
    const labelX = Math.cos(labelAngle) * radius * 0.7; // Position at 70% of radius
...
createInterHoleSpacingDimensions() {
    ...
    for (let i = 0; i < peripheralHoles.length; i++) {
        const currentHole = peripheralHoles[i];
        const nextHole = peripheralHoles[(i + 1) % peripheralHoles.length];
        
        // Calculate direction vector from current hole to next hole
        ...
        // Calculate perpendicular direction (flipped from before)
        const perpX = dirY;  // Changed from -dirY
        const perpY = -dirX; // Changed from dirX
        
        // Start points: one radius away from each hole center toward the other hole
        ...
        // Calculate how far to extend outward past cylinder
        // Calculate how far to extend outward past cylinder
const extensionLength = this.cylinderRadius * 0.3;
const overlapLength = 0.01; // How much the perpendicular lines extend past the connecting line

// End points of perpendicular lines (extend further)
...
// Connection points (slightly before the end points)
...
const connectMaterial = new THREE.LineBasicMaterial({ color: 0xff0000, linewidth: 2 });
...
// Calculate the actual spacing (center-to-center minus both radii)
const actualSpacing = distance - currentHole.radius - nextHole.radius;

// Place label just outside the connecting line
...
createCircleOutline(centerX, centerY, radius, z, dimension) {
    ...
    if (dimension) {const color = '0xd1d1d1'} else {color = '0x000000'}
