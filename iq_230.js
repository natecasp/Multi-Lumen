...
// Add a new error
    addError(id, title, description, buttonConfig = null) {
        ...
        errorElement.className = 'error-item new ' + id;
...
function calculate() {
  const checkMaterial = checkMaterial();
  const checkOD = checkOD();
  const checkLength = checkLength();
  const checkQuantity = checkQuantity();
  const checkSurfaceArea = checkSurfaceArea();
  const checkCentralHoleCompliance = checkCentralHoleCompliance();
  const checkHoleDistribution = checkHoleDistribution();
