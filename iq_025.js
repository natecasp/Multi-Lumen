...
generateInitialHoles(count) {
    ...
    if (peripheralHoles > 0) {
        ...
        if (this.includeCentralLumen) {
            ...
            const availableSpace = this.cylinderRadius - centralLumenRadius - this.minimumSeptum;
            
            // Place holes at the midpoint of available space
            placementDistance = centralLumenRadius + this.minimumSeptum + (availableSpace - optimalRadius - this.minimumSeptum) / 2;
        } else {
            // No central lumen, use existing logic
            placementDistance = Math.min(0.05, this.cylinderRadius - optimalRadius - this.minimumSeptum - 0.01);
        }
    ...
    this.holes.forEach((hole, index) => {
        ...
        let controlsHTML = `
          <div class="input-box">
            ...
            <input class="text-field-3 slider w-input" type="number" id="h${index}-diameter" value="${(hole.radius * 2).toFixed(4)}" min="0.015" max="0.15" step="0.001">
