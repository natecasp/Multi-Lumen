...
switchCamera() {
    ...
    if (cameraButton) {
        ...
        cameraButton.style.color = this.isPerspectiveView ? '#ffffff' : '#767676';
...
addCameraSwitchButton() {
    ...
    cameraButtonContainer.innerHTML = `
        <button id="camera-switch-btn" class="feet-tag" style="cursor:pointer; background:none;">SWITCH CAMERA (C)</button>
        <button id="print-mode-btn" class="feet-tag" style="cursor:pointer; background:none;">PRINT MODE (TAB)</button>
        <p class="feet-tag" id="shiftHold">SHIFT > DISTANCE</p>
        <p class="feet-tag" id="controlHold">CONTROL > DIAMETER</p>
        <p class="feet-tag" id="altHold">ALT > APPLY TO ALL</p>
    `;
...
togglePrintMode() {
    ...
    if (printButton) {
        ...
        printButton.style.color = this.isPrintMode ? '#ffffff' : '#767676';
    }
}
...
calculateMinimumCylinderRadius() {
    ...
    return Math.max(maxRequiredRadius, 0.025); // Scaled from 20 to 0.02
}
...
raycaster.ray.intersectPlane(this.workingPlane, this.intersectPoint);
    ...
    if (isAltPressed && isControlPressed) {
        $("#controlHold, #altHold").css("color", "#ffffff");
        ...
        const targetRadius = Math.max(0.0075, Math.min(currentMouseDistance * 0.5, 0.03));
        ...
        $("#shiftHold, #altHold").css("color", "#ffffff");
        ...
        const newRadius = Math.max(0.0075, Math.min(distanceFromHole, 0.03)); // Keep within reasonable bounds
        ...
        $("#shiftHold, #controlHold, #altHold").css("color", "#767676");
...
captureModelImage(captureWidth = 600, captureHeight = 600) {
    return new Promise((resolve) => {
        // Store current state
        const wasPrintMode = this.isPrintMode;
        const wasOrthographic = !this.isPerspectiveView;
        const originalCameraSettings = this.storeCameraSettings();
        const originalTextStyles = this.prepareForScreenshot(); // Add this line
        ...
        // Set consistent camera positioning
        this.setConsistentCameraForCapture();
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    ...
                    html2canvas(sandbox, {
                        ...
                        useCORS: true,
                        allowTaint: true,
                        scale: 1, // Use native resolution first
                        logging: false,
                        ...
                        const targetAspect = captureWidth / captureHeight; // Should be 1.0 for square
                        ...
                        const imageDataUrl = finalCanvas.toDataURL('image/png', 0.95); // High quality
                        
                        // Restore camera and previous state
                        this.restoreCameraSettings(originalCameraSettings);
                        
                        if (!wasPrintMode) {
                            this.exitPrintMode();
                        }
                        if (!wasOrthographic && !this.isPerspectiveView) {
                            this.switchCamera();
                        }
                        ...
                        // Restore state
                        this.restoreAfterScreenshot(originalTextStyles); // Add this line
                        this.restoreCameraSettings(originalCameraSettings);
                        if (!wasPrintMode) this.exitPrintMode();
                        if (!wasOrthographic && !this.isPerspectiveView) this.switchCamera();
                        
                        resolve(fallbackCanvas.toDataURL('image/png'));
                    });
                }, 400); // Longer delay for CSS2D positioning
            }, 200);
        }, 150);
    });
}
// Add this method to temporarily optimize for screenshots
prepareForScreenshot() {
    // Temporarily improve text rendering
    const css2dElement = this.css2dRenderer.domElement;
...
restoreAfterScreenshot(originalStyles) {
    const css2dElement = this.css2dRenderer.domElement;
...
// Add these methods to the CylinderWithHoles class
...
restoreCameraSettings(settings) {
    this.orthographicCamera.position.copy(settings.position);
    this.orthographicCamera.left = settings.left;
    this.orthographicCamera.right = settings.right;
    this.orthographicCamera.top = settings.top;
    this.orthographicCamera.bottom = settings.bottom;
...
// Add method to manually draw CSS2D elements
drawCss2dElementsScaled(ctx, captureWidth, captureHeight) {
    const sandboxRect = sandbox.getBoundingClientRect();
    const scaleX = captureWidth / sandboxRect.width;
    const scaleY = captureHeight / sandboxRect.height;
    const scale = Math.min(scaleX, scaleY);
    ...
    // Get all CSS2D objects from the scene
    this.scene.traverse((object) => {
        if (object.isCSS2DObject && object.visible) {
            ...
            // Convert to sandbox coordinates
            const sandboxX = (vector.x * 0.5 + 0.5) * sandboxRect.width;
            const sandboxY = (-vector.y * 0.5 + 0.5) * sandboxRect.height;
            
            // Scale to capture coordinates and apply offset
            const x = sandboxX * scale + offsetX;
            const y = sandboxY * scale + offsetY;
            
            // Only draw if within capture bounds
            if (x >= 0 && x <= captureWidth && y >= 0 && y <= captureHeight) {
                const element = object.element;
                const text = element.textContent.trim();
                
                if (text) {
                    // Scale font size based on capture resolution and scale
                    ...
                    if (bgColor && bgColor !== 'rgba(0, 0, 0, 0)' && bgColor !== 'transparent') {
                        ...
                        ctx.fillRect(x - textWidth/2 - padding, y - fontSize/2 - padding, 
                    ...
                    lines.forEach((line, index) => {
                        if (line.trim()) {
                            const lineY = y + (index - (lines.length - 1) / 2) * (fontSize + 2 * scale);
    ...
    console.log('Drew CSS2D elements with scale:', scale);
}

// Replace setOptimalCameraForThumbnail with this consistent version
setConsistentCameraForCapture() {
    // Calculate a consistent frustum size based on cylinder dimensions
    // This ensures the cylinder appears the same size regardless of current zoom
    
    // Base the frustum on cylinder radius plus space for dimensions
    const baseRadius = this.cylinderRadius;
    
    // Calculate maximum extent needed for all content
    ...
    // Add space for dimension lines and labels (they extend beyond the cylinder)
    const dimensionSpace = baseRadius * 0.6; // Space for dimension lines
    const labelSpace = baseRadius * 0.4;     // Additional space for labels
    
    const totalExtent = maxExtent + dimensionSpace + labelSpace;
    
    // Set a consistent frustum size with padding
    const padding = 1.2; // 20% padding
    const frustumSize = totalExtent * 2 * padding; // *2 because extent is radius, frustum is diameter
    
    // Get current sandbox aspect ratio
    const sandboxRect = sandbox.getBoundingClientRect();
    const aspectRatio = sandboxRect.width / sandboxRect.height;
    
    // Set orthographic camera bounds maintaining aspect ratio
    ...
    this.orthographicCamera.zoom = 1; // Reset zoom to 1 for consistency
    
    // Center camera on content
    let centerX = 0;
    let centerY = 0;
    
    // If we have holes, center on their centroid
    if (this.holes.length > 0) {
        ...
        this.holes.forEach(hole => {
            // Weight central holes less in centering calculation
    ...
    this.orthographicCamera.position.set(centerX, centerY, 0.3);
    this.orthographicCamera.lookAt(centerX, centerY, 0);
    this.orthographicCamera.updateProjectionMatrix();
    
    console.log('Consistent camera set:', {
        frustumSize,
        aspectRatio,
        bounds: {
            left: this.orthographicCamera.left,
...
const cylinderInstance = new CylinderWithHoles();
...
const thumbnailDataUrl = await cylinderInstance.captureModelImage(600, 600);
...
<div class="title-block"><p class="delete">x</p><p class="line-title">Line `+ lineItem +` (`+ unit +`)</p><p class="edit" id="`+ lineItem +`">Edit</p>${thumbnailHtml}</div>
