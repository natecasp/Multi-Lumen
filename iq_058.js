...
captureModelImage(width = 300, height = 300) {
    return new Promise((resolve) => {
        ...
        // Set optimal camera for thumbnail
        this.setOptimalCameraForThumbnail(1.0);
        ...
        // Wait longer for all elements to be ready
        setTimeout(() => {
            // Create final canvas
            ...
            console.log('WebGL Canvas dimensions:', glCanvas.width, 'x', glCanvas.height);
            console.log('WebGL Canvas style:', glCanvas.style.width, 'x', glCanvas.style.height);
            
            try {
                // Method 1: Direct canvas copy (works with preserveDrawingBuffer: true)
                if (glCanvas.width > 0 && glCanvas.height > 0) {
                    ...
                    if (canvasAspect > targetAspect) {
                        ...
                        sourceHeight = glCanvas.height;
                        sourceWidth = sourceHeight * targetAspect;
                        sourceX = (glCanvas.width - sourceWidth) / 2;
                        sourceY = 0;
                    } else {
                        // Canvas is taller, crop height
                        sourceWidth = glCanvas.width;
                        sourceHeight = sourceWidth / targetAspect;
                        sourceX = 0;
                        sourceY = (glCanvas.height - sourceHeight) / 2;
                    }
                    ...
                    console.log('Successfully drew WebGL canvas');
                } else {
                    throw new Error('WebGL canvas has zero dimensions');
                }
            } catch (error) {
                console.error('Error drawing WebGL canvas:', error);
                // Fallback: try readPixels approach
                this.captureUsingReadPixels(finalCtx, width, height);
            }
            
            // Add CSS2D elements if in print mode
            if (this.isPrintMode && this.css2dRenderer && 
                this.css2dRenderer.domElement.style.display !== 'none') {
                this.drawCss2dElementsManually(finalCtx, width, height);
            }
            
            // Get the final image
            const imageDataUrl = finalCanvas.toDataURL('image/png');
            
            // Debug: Check if we got a valid image (not just white)
            if (imageDataUrl === 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg==') {
                console.warn('Generated image appears to be blank');
            }
...
// Add fallback method using readPixels
captureUsingReadPixels(ctx, width, height) {
    try {
        ...
        // Render again with new viewport
        ...
        // Create ImageData and flip Y (WebGL coordinate system is flipped)
        ...
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                ...
                imageData.data[dstIndex + 3] = 255; // A - force opaque
            }
        }
        
        ctx.putImageData(imageData, 0, 0);
        console.log('Used readPixels fallback');
    } catch (error) {
        console.error('ReadPixels fallback also failed:', error);
        // Draw a placeholder
...
drawCss2dElementsManually(ctx, width, height) {
    const css2dElements = this.css2dRenderer.domElement.querySelectorAll('div');
    ...
    css2dElements.forEach(element => {
        ...
        // Get element position and style
        const rect = element.getBoundingClientRect();
        const rendererRect = this.css2dRenderer.domElement.getBoundingClientRect();
        const style = window.getComputedStyle(element);
        
        // Calculate position in thumbnail coordinates
        const x = (rect.left - rendererRect.left + rect.width / 2) * (width / rendererRect.width);
        const y = (rect.top - rendererRect.top + rect.height / 2) * (height / rendererRect.height);
        
        // Set text properties
        const fontSize = Math.max(8, parseInt(style.fontSize) * 0.8 || 10); // Scale down for thumbnail
...
// Add this method to set optimal camera position for thumbnails
setOptimalCameraForThumbnail(aspectRatio = 1.0) {
    // Calculate bounds including all content
    let minX = -this.cylinderRadius * 1.2;
    let maxX = this.cylinderRadius * 1.2;
    let minY = -this.cylinderRadius * 1.2;
    let maxY = this.cylinderRadius * 1.2;
    
    // Extend bounds for dimension labels
    if (this.dimensionLabels.length > 0) {
        minX = Math.min(minX, -this.cylinderRadius * 1.8);
        maxX = Math.max(maxX, this.cylinderRadius * 1.8);
        minY = Math.min(minY, -this.cylinderRadius * 1.8);
        maxY = Math.max(maxY, this.cylinderRadius * 1.8);
    }
    ...
    // Add padding
    const frustumSize = maxRange * 1.3;
    
    // Set orthographic camera
    this.orthographicCamera.left = -frustumSize * aspectRatio / 2;
    this.orthographicCamera.right = frustumSize * aspectRatio / 2;
...
const thumbnailDataUrl = await cylinderInstance.captureModelImage(200, 200);
