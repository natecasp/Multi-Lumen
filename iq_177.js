...
validateSemicircleRadius(newRadius) {
    ...
    if (newRadius > maxRadius) {
        return {
            valid: false,
            constrainedValue: this.semicircleRadius,
            reason: `Maximum radius is ${maxRadius.toFixed(3)}${unit} (maintains ${this.minimumSeptum.toFixed(3)}${unit} septum from outer wall)`
        };
    }
    
    // Check if any holes would be invalid with new semicircle radius
    for (let i = 0; i < this.holes.length; i++) {
        const hole = this.holes[i];
        const holeBottomEdge = hole.y - hole.radius;
        
        // Only check holes that could interfere with semicircle area
    ...
    // Minimum practical semicircle size
    const absoluteMinRadius = getBaseValues(unit).defaultWallThickness*2;
    if (newRadius < absoluteMinRadius) {
        return {
            valid: false,
...
const colors = ['Green', 'Blue', 'Red', 'Orange', 'Magenta', 'Cyan', 'Yellow', 'Purple', 'Light Green'];
  ...
  calculateOptimalDistance() {
    ...
    // Maximum distance from outer boundary
    const maxDistance = this.cylinderRadius - this.defaultHoleRadius - minSeptum;
    
    // Minimum distance to clear semicircle
    const minDistanceFromSemicircle = this.semicircleRadius + this.defaultHoleRadius + minSeptum;
    
    // Use basic minimum if no semicircle interference expected
    let minDistance = this.defaultHoleRadius + minSeptum;
    
    // For holes that might be placed in lower area, use semicircle constraint
    // This assumes new holes might be placed anywhere, so use the stricter constraint
    minDistance = Math.max(minDistance, minDistanceFromSemicircle);
    
    if (minDistance >= maxDistance) {
        console.warn('❌ calculateOptimalDistance: min >= max, using safe fallback');
        return Math.max(getBaseValues(unit).defaultWallThickness*3, this.cylinderRadius * 0.6);
    }
    
    // Place holes in the middle of available space, biased toward upper area
    const optimalDistance = minDistance + (maxDistance - minDistance) * 0.6;
    
    console.log(`✅ Optimal distance calculated: ${optimalDistance.toFixed(3)} (range: ${minDistance.toFixed(3)} - ${maxDistance.toFixed(3)})`);
...
updateSingleHole(holeIndex) {
 ...
 isValidPosition(x, y, radius, holeIndex) {
    ...
    const holeTopEdge = y + radius;
    
    
  
    
    if (holeBottomEdge <= minSeptum) {
        if (distanceFromCenter < minDistanceFromSemicircle) {
            const minDistanceFromSemicircle = this.semicircleRadius + radius + minSeptum;
            if (distanceFromCenter < minDistanceFromSemicircle) {
            console.log(`❌ Hole ${holeIndex}: too close to semicircle (distance: ${distanceFromCenter.toFixed(3)}, required: ${minDistanceFromSemicircle.toFixed(3)})`);
            return false;
        }
        }
    }
     if (y - radius < minSeptum) { // Hole bottom edge too close to y=0 line
        console.log(`❌ Hole ${holeIndex}: too close to semicircle midline (y-radius: ${(y-radius).toFixed(3)}, required: ${minSeptum.toFixed(3)})`);
        return false;
    }
...
updateSingleHoleDistanceLimits(holeIndex) {
    ...
    // Maximum distance: limited by outer cylinder
    const maxDistance = this.cylinderRadius - hole.radius - minSeptum;
    
    // Minimum distance: must clear semicircle + maintain septum
    let minDistance = hole.radius + minSeptum;
    
    // If hole could interfere with semicircle area, use stricter constraint
    const holeBottomEdge = hole.y - hole.radius;
    if (holeBottomEdge <= minSeptum) {
        // Hole is in or near semicircle area - must maintain distance from semicircle
        const minDistanceFromSemicircle = this.semicircleRadius + hole.radius + minSeptum;
        minDistance = Math.max(minDistance, minDistanceFromSemicircle);
        
        console.log(`🔄 Hole ${holeIndex} in semicircle area - min distance: ${minDistance.toFixed(3)}`);
    }
    ...
    if (distanceSlider && distanceInput) {
        ...
        if (hole.distance < minDistance || hole.distance > maxDistance) {
            const clampedDistance = Math.max(minDistance, Math.min(maxDistance, hole.distance));
            
            // Update hole position
            const { x, y } = this.polarToCartesian(hole.angle, clampedDistance);
            hole.distance = clampedDistance;
            hole.x = x;
            hole.y = y;
            
            console.log(`🔧 Clamped hole ${holeIndex} distance to ${clampedDistance.toFixed(3)}`);
        }
...
createSmileHoleUI() {
    ...
    this.holes.forEach((hole, index) => {
        ...
        // FIXED: Calculate constraints properly using available space
        const cylinderInnerEdge = this.cylinderRadius - spacingConstraint; 
        const semicircleOuterEdge = this.semicircleRadius + spacingConstraint; 
        const holeRadius = hole.radius;
        ...
        // Minimum distance based on available space constraint
        let minDistance = holeRadius + spacingConstraint; // Basic minimum using available space
        ...
        if (minDistance >= maxDistance) {
            console.error(`❌ Invalid range for hole ${index}: min=${minDistance.toFixed(3)} >= max=${maxDistance.toFixed(3)}`);
            // Adjust using available space as fallback
            minDistance = Math.max(getBaseValues(unit).defaultWallThickness*2, maxDistance - getBaseValues(unit).defaultWallThickness*2);
        }
