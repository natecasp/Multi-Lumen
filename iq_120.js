...
function updateCameraConstraints(unit) {
    ...
    const scaleFactor = unit === 'mm' ? UNIT_CONVERSIONS.INCH_TO_MM : UNIT_CONVERSIONS.MM_TO_INCH;
    
    if (globalPerspectiveCamera) {
        globalPerspectiveCamera.position.multiplyScalar(scaleFactor);
        globalPerspectiveCamera.updateProjectionMatrix();
    }
    
    if (globalOrthographicCamera) {
        globalOrthographicCamera.position.multiplyScalar(scaleFactor);
        globalOrthographicCamera.updateProjectionMatrix();
    }
...
globalScene.add(backRightLight);
    ...
    globalOrthographicCamera.position.set(0, 0, getBaseValues(unit).defaultCylinderRadius*3);
...
function setupCameraSwitching() {
    ...
    globalOrthographicCamera.position.set(centerX, centerY, getBaseValues(unit).defaultCylinderRadius*3);
...
class CrescentSystem {
  ...
  setConsistentCameraForCapture() {
    ...
    globalOrthographicCamera.position.set(0, centerY, getBaseValues(unit).defaultCylinderRadius*3);
