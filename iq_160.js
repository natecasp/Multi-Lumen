...
function checkAndAddCentralHole() {
  if(window.profileManager.currentSystem instanceof CircularHolesSystem) {
    ...
    // This means: hole distance >= 2 * wall thickness
    const outerRadius = system.cylinderRadius;
    let needsCentralHole = false;
    
    peripheralHoles.forEach(hole => {
      const holeDistance = hole.distance;
      const wallThickness = outerRadius - holeDistance;
      
      if (holeDistance >= 2 * wallThickness) {
        needsCentralHole = true;
      }
    });
    
    if (needsCentralHole) {
      console.log('Adding central hole - holes are too far from center');
      
      // Calculate the central hole size
      // We want: distance from central hole edge to peripheral holes = wall thickness
      // So: central hole radius = hole distance - wall thickness
      const representativeHole = peripheralHoles[0]; // Use first peripheral hole as reference
      ...
      const unit = $("#unit").val() || 'in';
      const minCentralRadius = getBaseValues(unit).minimumHoleRadius * 2; // Minimum size
      const maxCentralRadius = representativeHole.distance * 0.8; // Don't make it too big
      ...
      // Update hole count in UI (add 1 for the central hole)
      const holeCountInput = document.getElementById('hole-count');
      if (holeCountInput) {
        const newCount = Math.min(9, system.holes.length + 1); // Cap at 9 total
      ...
      notification.innerHTML = `
        ...
        <div>Holes were too far from center. Added central lumen for optimal wall thickness.</div>
      `;
      document.body.appendChild(notification);
