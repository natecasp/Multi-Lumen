...
function updateCameraConstraints(unit) {
    ...
    // Update orthographic camera frustum - FIXED: Proper scaling for units
    if (globalOrthographicCamera) {
        ...
        // CRITICAL: Set camera position based on unit scale
        // The camera needs to be positioned appropriately for each unit system
        const targetDistance = unit === 'mm' ? 
            10.16 : // For mm, position camera at 10.16mm (0.4 inches converted)
            0.4;    // For inches, use 0.4 inches
        
        // Preserve the camera's current direction but adjust distance
        const currentPosition = globalOrthographicCamera.position.clone();
        const currentDistance = currentPosition.length();
        
        if (currentDistance > 0) {
            // Normalize and scale to appropriate distance for the unit
            currentPosition.normalize().multiplyScalar(targetDistance);
            globalOrthographicCamera.position.copy(currentPosition);
        } else {
    ...
    if (globalPerspectiveCamera) {
        const targetDistance = unit === 'mm' ? 
            10.16 : // For mm, position camera at 10.16mm
            0.4;    // For inches, use 0.4 inches
        
        const currentPosition = globalPerspectiveCamera.position.clone();
        const currentDistance = currentPosition.length();
        
        if (currentDistance > 0) {
            currentPosition.normalize().multiplyScalar(targetDistance);
            globalPerspectiveCamera.position.copy(currentPosition);
        } else {
    ...
    console.log(`Camera updated for ${unit}: frustum=${baseValues.cameraFrustumSize}, distance=${targetDistance}`);
}
