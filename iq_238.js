...
function checkMaterial() {
  if (['Pebax 72D','Pebax 63D', 'Pebax 55D','Pebax 45D','Vestamid ML21', 'Pellethane 55D', 'Pellethane 65D'].includes($("#material-2").val())) {
    ...
    addError(
            'material',
            'Please select a material to continue with your quote',
            'Select a material.'
        );
    return false;
  }
...
function checkOD() {
  ...
  if(Number($("#cylinder-diameter-input").val()) >=  getBaseValues(unit).minCylinderDiameter && Number($("#cylinder-diameter-input").val()) <=  getBaseValues(unit).maxGreenCylinderDiameter ) {
    removeError('od-small');
    removeError('od-large');'
    return true;
  } else {
    if (Number($("#cylinder-diameter-input").val()) <  getBaseValues(unit).minCylinderDiameter) {
    addError(
            'od-small',
            'Outer diameter is too small to qualify for an instant quote.',
            'Increase outer diameter.'
        );
    }
    if(Number($("#cylinder-diameter-input").val()) >  getBaseValues(unit).maxGreenCylinderDiameter) {
      addError(
            'od-large',
            'Outer diameter is too large to qualify for an instant quote.',
...
function checkLength() {
  ...
  if(Number($("#length-2").val()) >= getBaseValues(unit).minGreenlightLength && Number($("#length-2").val()) <= getBaseValues(unit).maxGreenlightLength) {
    removeError('length-small');
    removeError('length-large');
    return true;
  } else {
    if(Number($("#length-2").val()) < getBaseValues(unit).minGreenlightLength) {
      addError(
            'length-small',
            'Extrusion length is too small to qualify for an instant quote.',
            'Increase length.'
        ); 
    }
    if(Number($("#length-2").val()) > getBaseValues(unit).maxGreenlightLength) {
      addError(
            'length-large',
            'Extrusion length is too large to qualify for an instant quote.',
            'Decrease length.'
        ); 
    }
    return false;
...
function checkQuantity() {
  if(Number($("#quantity-2").val()) <= 2500) {
    ...
    addError(
            'qty',
            'Max quantity for an instant quote is 2,500 feet.',
            'Decrease quantity, or continue with custom quote.'
        ); 
    return false;
  }
...
function checkSurfaceArea() {
    ...
    if (ratio >= maxRatio && ratio <= minRatio) {
      removeError('surface-area');
      return true;
    } else {
      addError(
            'surface-area',
            'Too much surface area, please increase lumen size or decrease OD to qualify for an instant quote.',
            'Increase lumen size or decrease OD.'
        );
      return false;
    }
