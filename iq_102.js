//IQueue and its associated code are the exclusive rights of Midwest Interventional Systems, DO NOT USE OR COPY
...
// Global unit conversion factor (UNC)
let UNC = 1; // Default to 1 (inches)
let currentDisplayUnit = 'in'; // Track current display unit
...
function setupCameraControls() {
    ...
    // Get current unit for proper constraints
    const currentUnit = $("#unit").val() || 'in';
    
    if (isPerspectiveView) {
        // Perspective camera settings (3D view)
        globalControls.enableRotate = true;
        
        // Unit-aware distance constraints
        if (currentUnit === 'mm') {
            globalControls.minDistance = 0.05 * INCH_TO_MM;
            globalControls.maxDistance = 1 * INCH_TO_MM;
        } else {
...
function switchCamera() {
    ...
    const currentUnit = $("#unit").val() || 'in';
    const scale = currentUnit === 'mm' ? INCH_TO_MM : 1;
    ...
    if (isPerspectiveView) {
        ...
        // Reset orthographic camera to perfect top view with unit-aware positioning
        globalOrthographicCamera.position.set(0, 0, 0.3 * scale);
        globalOrthographicCamera.lookAt(0, 0, 0);
...
function setupCameraSwitching() {
    ...
    this.cylinderRadius = 0.1 * UNC; // Default: 0.200m diameter
    this.cylinderHeight = 0.15 * UNC;
    this.minimumSeptum = 0.005 * UNC;
    this.includeCentralLumen = false; // Default: no central lumen
    this.centralLumenRadius = 0.0125 * UNC; // Default: 0.025m diameter
    ...
    const currentUnit = $("#unit").val() || 'in';
    const unitScale = currentUnit === 'mm' ? INCH_TO_MM : 1;
    ...
    const cameraDistance = 0.3 * unitScale;
    globalOrthographicCamera.position.set(centerX, centerY, cameraDistance);
...
async captureModelImage(width = 400, height = 400) {
  ...
  updateHoleDiameterLimits() {
    ...
    if (this.holes.length === 0) return 0.02 * UNC;
    ...
    return Math.max(maxRequiredRadius, 0.025*UNC);
}
    ...
    if (centralHoleIndex >= 0) {
        ...
        this.regenerateHoles(peripheralHoleCount + 1);
            ...
            isValidPosition(x, y, radius, holeIndex) {
                ...
                if (distanceFromCenter < 0.001*UNC) {
 ...
 generateInitialHoles(count) {
    ...
    if (peripheralHoles > 0) {
        ...
        holeRadius = Math.max(holeRadius, 0.005*UNC); 
    ...
    this.initializePositionHistory();
            ...
            calculateOptimalRadius(holeCount) {
                if (holeCount <= 2) return 0.02*UNC; // Default radius
                ...
                const optimalRadius = Math.min(
                    ...
                    0.025*UNC // Maximum hole radius
                );
                
                return Math.max(0.005*UNC, Math.min(optimalRadius, 0.025*UNC));
            }
  ...
  findClosestValidRadius(x, y, targetRadius, holeIndex) {
    ...
    const minRadius = 0.0075*UNC; // Keep general min
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (isCentralHole) {
       ...
       newRadius = Math.max(0.0075*UNC, Math.min(distanceFromHole, this.cylinderRadius));
    } else {
        newRadius = Math.max(0.0075*UNC, Math.min(distanceFromHole, this.cylinderRadius));
    }
...
const material = new THREE.MeshStandardMaterial({ 
    ...
    opacity: 1
            ...
            createHoleMarkers() {
                ...
                this.holes.forEach((hole, index) => {
                    ...
                    const controlGeometry = new THREE.SphereGeometry(0.008*UNC, 16, 16);
    ...
    <div class="input-group">
        ...
        <input class="range-slider c-${hole.color}" type="range" min="${0.015*UNC}" max="${maxHoleDiameter.toFixed(3)}" value="${(hole.radius * 2).toFixed(4)}" step="${0.001*UNC}" id="hole-${index}-diameter-range" >
        <input type="number" id="hole-${index}-diameter" value="${(hole.radius * 2).toFixed(4)}" min="${0.015*UNC}" max="${maxHoleDiameter.toFixed(3)}" step="${0.001*UNC}">
    ...
    `;
        ...
        <div class="input-group">
            ...
            <input type="range" class="range-slider c-${hole.color}" min="0" max="${(this.cylinderRadius * 0.8).toFixed(3)}" value="${hole.distance.toFixed(4)}" step="${0.001*UNC}" id="hole-${index}-distance-range">
            <input type="number" id="hole-${index}-distance" min="0" max="${(this.cylinderRadius * 0.8).toFixed(3)}" value="${hole.distance.toFixed(4)}" step="${0.001*UNC}">
        </div>
        
        <div class="input-group">
            <label>Angle:</label>
            <input type="range" class="range-slider c-${hole.color}" min="0" max="360" value="${Math.round(hole.angle)}" step="1" id="hole-${index}-angle-range">
...
createCircularPrintElements() {
    ...
    const z = this.cylinderHeight / 2 + 0.002*UNC;
...
createHoleDimensionLines(hole, index) {
    const z = this.cylinderHeight / 2 + 0.002*UNC;
    const extensionLength = 0.03*UNC;
        ...
        if (holeBottomEdge <= this.minimumSeptum) { // Allow some tolerance above y=0
            ...
            if (hole.distance < requiredMinDistance) {
                ...
                return {
                    valid: false,
                    constrainedValue: this.semicircleRadius, // Keep current value
                    reason: `Would intersect with ${hole.name} (in lower area) - maximum radius: ${Math.max(0, maxAllowedRadius).toFixed(3)}${unit}`
                };
            }
        }
    }
 ...
 createSmileHoleUI() {
    ...
    this.holes.forEach((hole, index) => {
        ...
        div.innerHTML = `
            ...
            <div class="input-group">
                <label>Diameter:</label>
                <input type="range" class="range-slider c-${hole.color}" id="smile-hole-${index}-diameter-range" min="0.005" max="${maxHoleDiameter.toFixed(3)}" value="${(hole.radius * 2).toFixed(4)}" step="0.001">
            ...
            <div class="input-group">
                <label>Distance:</label>
                <input type="range" class="range-slider c-${hole.color}" id="smile-hole-${index}-distance-range" min="${minDistance.toFixed(3)}" max="${maxDistance.toFixed(3)}" value="${useDistance.toFixed(4)}" step="0.001">
...
createSmileHoleUI() {
    ...
    this.holes.forEach((hole, index) => {
        ...
        div.innerHTML = `
            ...
            <div class="input-group">
                <label>Diameter:</label>
                <input type="range" class="range-slider c-${hole.color}" id="smile-hole-${index}-diameter-range" min="0.005" max="0.06" value="${(hole.radius * 2).toFixed(4)}" step="0.001">
                <input type="number" id="smile-hole-${index}-diameter" min="0.005" max="0.06" value="${(hole.radius * 2).toFixed(4)}" step="0.001">
            </div>
            <div class="input-group">
                <label>Angle:</label>
                <input type="range" class="range-slider c-${hole.color}" id="smile-hole-${index}-angle-range" min="0" max="360" value="${Math.round(hole.angle)}" step="1">
...
if (distanceInput) {
    ...
    createSmileHoleDimensionLines(hole, index) {
      ...
      const dimLine = new THREE.Line(dimLineGeometry, dimLineMaterial);
       ...
       const css2dLabel = new CSS2DObject(labelDiv);
       css2dLabel.position.set(endX, endY, z);
   ...
   createSemicircleDimensions(z) {
       ...
       label.position.set(endX, endY, z);
...
cleanupCrescentUI() {
    // Hide crescent-specific controls that might not be caught by the main display:none
    const crescentSpecificElements = [
        '#even-wall-thickness-checkbox',
        '#circular-radius',
        '#circular-radius-input', 
        '#crescent-corner-radius',
        '#crescent-corner-radius-input',
        '#crescent-septum-thickness',
        '#crescent-septum-thickness-input'
    ];
    
    crescentSpecificElements.forEach(selector => {
        const element = document.querySelector(selector);
        if (element && element.closest('#crescent-controls')) {
            // Only hide if it's within the crescent controls container
            element.style.display = 'none';
        }
    });
    
    // Reset any crescent-specific checkboxes
    const evenWallCheckbox = document.getElementById('even-wall-thickness-checkbox');
    if (evenWallCheckbox) {
        evenWallCheckbox.checked = true; // Reset to default
    }
    
    // Clear any crescent-specific validation styling
    crescentSpecificElements.forEach(selector => {
        const element = document.querySelector(selector);
        if (element) {
...
$("#unit").on('change', function(e, previousUnit) {
    const newUnit = $(this).val();
    const oldUnit = previousUnit || (newUnit === 'mm' ? 'in' : 'mm');
    
    console.log(`Unit change: ${oldUnit} → ${newUnit}`);
    ...
    // Update all UI values to display in new units
    updateUIForUnitChange(oldUnit, newUnit);
    
    // Update camera for new scale
    adjustCameraForUnitChange(oldUnit, newUnit);
    
    // Update system-specific limits that depend on units
    if (window.profileManager && window.profileManager.currentSystem) {
        const system = window.profileManager.currentSystem;
        
        // Update limits for different systems
        if (system instanceof CircularHolesSystem) {
            ...
            system.updateHolePositionLimitsUI();
            system.updateHoleDiameterLimits();
    ...
    const step = `Unit changed from ${oldUnit} to ${newUnit}`;
    updateJourney(step);
});
              // Convert from internal inches to display units
function internalToDisplay(inchValue) {
    return inchValue * globalUnitConversionFactor;
}

// Convert from display units to internal inches
function displayToInternal(displayValue) {
    return displayValue / globalUnitConversionFactor;
}

// Get appropriate precision for current display unit
function getDisplayPrecision() {
    return currentDisplayUnit === 'mm' ? 2 : 3;
}

// Get appropriate step value for current display unit
function getDisplayStep(baseStep = 0.001) {
    return currentDisplayUnit === 'mm' ? (baseStep * INCH_TO_MM).toFixed(4) : baseStep.toFixed(3);
}

// Update UI values for unit change (doesn't change internal values)
function updateUIForUnitChange(fromUnit, toUnit) {
    if (fromUnit === toUnit) return;
    
    const conversionFactor = (fromUnit === 'in' && toUnit === 'mm') ? INCH_TO_MM : MM_TO_INCH;
    
    // Convert all unit-convertible elements
    $('.unit-convertible').each(function() {
        const $element = $(this);
        ...
        if (!isNaN(currentValue)) {
            ...
            $element.val(convertedValue.toFixed(precision));
            
            // Update min, max, step attributes
            updateElementConstraints($element, conversionFactor);
        }
    });
    
    // Convert dynamically created hole controls
    convertDynamicHoleControlsUI(conversionFactor);
}

// Update element constraints (min, max, step)
function updateElementConstraints($element, conversionFactor) {
    // Update min attribute
    if ($element.attr('min')) {
        const minValue = parseFloat($element.attr('min'));
        if (!isNaN(minValue)) {
            const convertedMin = minValue * conversionFactor;
            $element.attr('min', convertedMin.toFixed(getDisplayPrecision()));
        }
    }
    
    // Update max attribute
    if ($element.attr('max')) {
        const maxValue = parseFloat($element.attr('max'));
        if (!isNaN(maxValue)) {
            const convertedMax = maxValue * conversionFactor;
            $element.attr('max', convertedMax.toFixed(getDisplayPrecision()));
        }
    }
    
    // Update step attribute
    if ($element.attr('step')) {
        const currentStep = parseFloat($element.attr('step'));
        if (!isNaN(currentStep)) {
            const newStep = currentStep * conversionFactor;
            $element.attr('step', newStep.toFixed(4));
        }
    }
}

// Convert dynamic hole controls UI only
function convertDynamicHoleControlsUI(conversionFactor) {
    const selectors = [
        '[id^="hole-"][id$="-diameter-range"]',
        '[id^="hole-"][id$="-diameter"]', 
        '[id^="hole-"][id$="-distance-range"]',
        '[id^="hole-"][id$="-distance"]',
        '[id^="smile-hole-"][id$="-diameter-range"]',
        '[id^="smile-hole-"][id$="-diameter"]',
        '[id^="smile-hole-"][id$="-distance-range"]',
        '[id^="smile-hole-"][id$="-distance"]'
    ];
...
function createOuterDiameterDimension(system, z) {
    ...
    const extensionDistance = outerRadius + 0.05;
    ...
    // Add perpendicular cross line at the cylinder edge
    const crossLength = outerRadius * 0.05;
    const perpAngle = bestAngle + Math.PI/2;
    const crossStart = {
        x: startX + Math.cos(perpAngle) * crossLength,
        y: startY + Math.sin(perpAngle) * crossLength
    };
    const crossEnd = {
        x: startX - Math.cos(perpAngle) * crossLength,
        y: startY - Math.sin(perpAngle) * crossLength
    ...
    globalScene.add(crossLine);
    system.dimensionLines.push(crossLine);
    ...
    labelDiv.style.cssText = `
        ...
        white-space: nowrap;
    `;
    labelDiv.innerHTML = `<div><strong>Outer Diameter</strong></div><div>⌀ ${outerDiameter}${unit}</div>`;
    
    const css2dLabel = new CSS2DObject(labelDiv);
    css2dLabel.position.set(endX + 0.01, endY + 0.01, z);
...
function populateCircularProfile(lineNumber) {
    ...
    if (hasCentralLumen) {
        const centralDiameter = parseFloat($holeLayouts.first().find('input').first().val());
        $("#inner-diameter, #inner-diameter-input").val(centralDiameter.toFixed(3));
    }
    
    // Set hole count and apply
    $("#hole-count").val($holeLayouts.length);
    $("#apply-holes").click();
    
    // Wait for holes to be created, then populate data
    setTimeout(() => {
        const system = window.profileManager.currentSystem;
        if (system instanceof CircularHolesSystem) {
            $holeLayouts.each(function(index) {
                const $inputs = $(this).find('input');
                if ($inputs.length >= 3) { // Not central lumen
                    ...
                    if (system.holes[index]) {
                        system.holes[index].radius = diameter / 2;
                        system.holes[index].distance = distance;
                        system.holes[index].angle = angle;
                        
                        const { x, y } = system.polarToCartesian(angle, distance);
...
function updateCircularProfileData(line) {
    const system = window.profileManager.currentSystem;
    const profileData = getCircularProfileData();
...
function convertValue(value, fromUnit, toUnit) {
    ...
    if (fromUnit === 'in' && toUnit === 'mm') {
        ...
        return numValue * MM_TO_INCH;
    }
    return value;
}

// Get appropriate step value for unit
function getStepForUnit(unit, baseStep = 0.001) {
    return unit === 'mm' ? (baseStep * INCH_TO_MM).toFixed(4) : baseStep.toFixed(3);
}

// Get appropriate precision for display
function getPrecisionForUnit(unit) {
    return unit === 'mm' ? 2 : 3;
}

// Convert all unit-convertible elements
function convertAllDimensions(fromUnit, toUnit) {
    if (fromUnit === toUnit) return;
    
    // Convert static elements with unit-convertible class
    $('.unit-convertible').each(function() {
        const $element = $(this);
        const currentValue = parseFloat($element.val());
        ...
        if (!isNaN(currentValue)) {
           // Update min, max, step attributes
            updateElementConstraints($element, fromUnit, toUnit);
            const convertedValue = convertValue(currentValue, fromUnit, toUnit);
            const precision = getPrecisionForUnit(toUnit);
            $element.val(convertedValue.toFixed(precision));
    ...
    // Convert dynamically created hole controls
    convertDynamicHoleControls(fromUnit, toUnit);
    
    // Update system internal values
    updateSystemInternalValues(fromUnit, toUnit);

    // Adjust camera for the new unit scale
    adjustCameraForUnitChange(fromUnit, toUnit);
}

// Update min, max, step attributes for an element
function updateElementConstraints($element, fromUnit, toUnit) {
    ...
    if ($element.attr('min')) {
        const minValue = parseFloat($element.attr('min'));
        if (!isNaN(minValue)) {
            const convertedMin = convertValue(minValue, fromUnit, toUnit);
            $element.attr('min', convertedMin.toFixed(getPrecisionForUnit(toUnit)));
        }
    }
    ...
    if ($element.attr('max')) {
        const maxValue = parseFloat($element.attr('max'));
        if (!isNaN(maxValue)) {
            const convertedMax = convertValue(maxValue, fromUnit, toUnit);
            $element.attr('max', convertedMax.toFixed(getPrecisionForUnit(toUnit)));
        }
    }
    ...
    if ($element.attr('step')) {
        const currentStep = parseFloat($element.attr('step'));
        if (!isNaN(currentStep)) {
            const newStep = getStepForUnit(toUnit, currentStep);
            $element.attr('step', newStep);
        }
    }
}

// Convert dynamically created hole controls
function convertDynamicHoleControls(fromUnit, toUnit) {
    // Convert circular hole controls
    $('[id^="hole-"][id$="-diameter-range"], [id^="hole-"][id$="-diameter"], [id^="hole-"][id$="-distance-range"], [id^="hole-"][id$="-distance"]').each(function() {
        const $element = $(this);
        const currentValue = parseFloat($element.val());
    ...
    // Convert smile hole controls
    $('[id^="smile-hole-"][id$="-diameter-range"], [id^="smile-hole-"][id$="-diameter"], [id^="smile-hole-"][id$="-distance-range"], [id^="smile-hole-"][id$="-distance"]').each(function() {
        const $element = $(this);
        const currentValue = parseFloat($element.val());
...
// Universal geometry update function
function updateSystemGeometry(system) {
    if (!system) return;
    
    try {
        if (system instanceof CircularHolesSystem) {
            system.createCylinder();
            if (system.createHoleMarkers) system.createHoleMarkers();
            if (system.createHoleUI) system.createHoleUI();
        } else if (system instanceof PieSliceSystem) {
            system.updateGeometry();
        } else if (system instanceof CrescentSystem) {
            system.updateCylinderGeometry();
            if (system.updateControllerPosition) system.updateControllerPosition();
        } else if (system instanceof SmileSystem) {
            system.updateCylinderGeometry();
            if (system.createHoleMarkers) system.createHoleMarkers();
            if (system.createPolarControls) system.createPolarControls();
        } else {
            console.warn('Unknown system type for geometry update:', system.constructor.name);
        }
    } catch (error) {
        console.error('Error updating system geometry:', error);
    }
}
// Update internal system values (simplified version)
function updateSystemInternalValues(fromUnit, toUnit) {
    if (!window.profileManager || !window.profileManager.currentSystem) return;
    
    const system = window.profileManager.currentSystem;
    
    // Convert common properties
    if (system.cylinderRadius !== undefined) {
        system.cylinderRadius = convertValue(system.cylinderRadius, fromUnit, toUnit);
    }
    
    if (system.cylinderHeight !== undefined) {
        system.cylinderHeight = convertValue(system.cylinderHeight, fromUnit, toUnit);
    }
    
    if (system.minimumSeptum !== undefined) {
        system.minimumSeptum = convertValue(system.minimumSeptum, fromUnit, toUnit);
    }
    
    // Convert system-specific properties
    if (system instanceof CircularHolesSystem) {
        if (system.centralLumenRadius !== undefined) {
            system.centralLumenRadius = convertValue(system.centralLumenRadius, fromUnit, toUnit);
        }
        
        // Convert hole positions and sizes
        if (system.holes) {
            system.holes.forEach(hole => {
    ...
    // Update geometry using universal method
    updateSystemGeometry(system);
    
    console.log(`Updated ${system.constructor.name} internal values from ${fromUnit} to ${toUnit}`);
}
// Camera adjustment for unit changes
function adjustCameraForUnitChange(fromUnit, toUnit) {
    if (fromUnit === toUnit) return;
    
    const conversionFactor = (fromUnit === 'in' && toUnit === 'mm') ? INCH_TO_MM : MM_TO_INCH;
    
    // Store current camera state for both cameras
    const perspectiveCameraState = {
        position: globalPerspectiveCamera.position.clone(),
        target: globalControls.target.clone()
    };
    
    const orthographicCameraState = {
        position: globalOrthographicCamera.position.clone(),
        left: globalOrthographicCamera.left,
        right: globalOrthographicCamera.right,
        top: globalOrthographicCamera.top,
    ...
    globalPerspectiveCamera.position.multiplyScalar(conversionFactor);
    
    // Adjust orthographic camera position and frustum
    globalOrthographicCamera.position.multiplyScalar(conversionFactor);
    globalOrthographicCamera.left *= conversionFactor;
    globalOrthographicCamera.right *= conversionFactor;
    globalOrthographicCamera.top *= conversionFactor;
    globalOrthographicCamera.bottom *= conversionFactor;
    
    // Adjust controls target
    globalControls.target.multiplyScalar(conversionFactor);
    ...
    console.log(`Camera adjusted for unit change: ${fromUnit} → ${toUnit}, factor: ${conversionFactor}`);
}

// Update camera constraints based on unit
function updateCameraConstraints(unit) {
    if (unit === 'mm') {
        // Constraints for mm (larger scale)
        if (isPerspectiveView) {
            globalControls.minDistance = 0.05 * INCH_TO_MM; // ~1.27mm
            globalControls.maxDistance = 1 * INCH_TO_MM;    // ~25.4mm
        } else {
            globalControls.minZoom = 0.5;
            globalControls.maxZoom = 5;
        }
    } else {
        // Constraints for inches (original scale)
        if (isPerspectiveView) {
            globalControls.minDistance = 0.05;
            globalControls.maxDistance = 1;
        } else {
...
// Update camera frustum size for consistent view
function updateCameraFrustumForUnit(unit) {
    const aspect = sandbox.offsetWidth / sandbox.offsetHeight;
    
    if (unit === 'mm') {
        // Larger frustum for mm units
        const frustumSize = 0.4 * INCH_TO_MM; // Scale up the frustum
        globalOrthographicCamera.left = -frustumSize * aspect / 2;
        globalOrthographicCamera.right = frustumSize * aspect / 2;
        globalOrthographicCamera.top = frustumSize / 2;
        globalOrthographicCamera.bottom = -frustumSize / 2;
    } else {
        // Original frustum for inch units
        const frustumSize = 0.4;
        globalOrthographicCamera.left = -frustumSize * aspect / 2;
        globalOrthographicCamera.right = frustumSize * aspect / 2;
        globalOrthographicCamera.top = frustumSize / 2;
