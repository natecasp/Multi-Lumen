...
createArcDimension(peripheralHoles, z, unit, holeCenterRadius) {
    ...
    // Extension distance from hole centers outward
    const extensionDistance = holeCenterRadius + 0.03;
    ...
    // Calculate the spacing and create label
    const dx = secondHole.x - firstHole.x;
    const dy = secondHole.y - firstHole.y;
    const centerDistance = Math.sqrt(dx * dx + dy * dy);
    const actualSpacing = centerDistance - firstHole.radius - secondHole.radius;
...
createInterHoleSpacingDimensions() {
    ...
    // If holes are uniform, only create dimension between first two holes
    const isUniform = this.arePeripheralHolesUniform();
    const iterationCount = isUniform ? 1 : 
                          (peripheralHoles.length === 2 ? 1 : peripheralHoles.length);
    ...
    for (let i = 0; i < iterationCount; i++) {
        ...
        const overlapLength = 0.005; // How much the perpendicular lines extend past the connecting line
