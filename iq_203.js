...
createHoleCenterCircle() {
    ...
    for (let i = 0; i < peripheralHoles.length; i++) {
        ...
        // Calculate midpoint between holes
        const midX = (currentHole.x + nextHole.x) / 2;
        const midY = (currentHole.y + nextHole.y) / 2;
        ...
        const perpUnitX = perpX / perpLength;
        const perpUnitY = perpY / perpLength;
        
        // Extend line beyond cylinder radius
        const extensionLength = this.cylinderRadius * 1.3;
        const startX = midX + perpUnitX * extensionLength;
        const startY = midY + perpUnitY * extensionLength;
        const endX = midX - perpUnitX * extensionLength;
        const endY = midY - perpUnitY * extensionLength;
        ...
        this.printModeElements.push(line);
        
        // Calculate and display spacing distance
        const spacing = Math.sqrt(dx * dx + dy * dy);
        this.createSpacingLabel(spacing, startX, startY, z, unit);
    }
}
