...
async captureModelImage(width = 400, height = 400) {
 ...
 generateInitialHoles(count) {
    ...
    if (peripheralHoles > 0) {
        ...
        holeRadius = Math.max(holeRadius, getBaseValues(unit).minimumHoleRadius * 2); // 5mm minimum
...
const material = new THREE.MeshStandardMaterial({ 
    ...
    `;
        ...
        <div class="input-group">
            ...
            <input type="range" class="range-slider c-${hole.color}" min="0" max="${(this.cylinderRadius * 0.8).toFixed(3)}" value="${hole.distance.toFixed(4)}" step="${getStepValue(unit, 'diameter')}" id="hole-${index}-distance-range">
            <input type="number" id="hole-${index}-distance" min="0" max="${(this.cylinderRadius * 0.8).toFixed(3)}" value="${hole.distance.toFixed(4)}" step="${getStepValue(unit, 'diameter')}">
        </div>
...
calculateMinimumOuterDiameter() {
  const unit = $("#unit").val();
    let minRadius = getBaseValues(unit).minCylinderDiameter / 2; // NEW: Use 0.079m minimum diameter instead of 0.05
    ...
    return Math.max(getBaseValues(unit).minCylinderDiameter, minRadius * 2); // NEW: Ensure minimum 0.079m diameter
}
...
function populateCircularProfile(lineNumber) {function populateCircularProfile(lineNumber) {
    ...
    // Set the checkbox (this will trigger applyHoleChanges via the event listener)
    $("#central-lumen-checkbox").prop('checked', hasCentralLumen).trigger('change');
    
    // Set hole count (this will also trigger applyHoleChanges via the event listener)
    $("#hole-count").val($holeLayouts.length).trigger('input');
    
    // Wait for the system to regenerate holes, then populate the data
    setTimeout(() => {
        populateCircularHoleDataFromLine($holeLayouts);
    }, 800);
}
function populateCircularHoleDataFromLine($holeLayouts) {
    const system = window.profileManager.currentSystem;
    ...
    $holeLayouts.each(function(lineIndex) {
        ...
        if (label === 'Central Lumen') {
            // Central lumen should be holes[0]
            ...
            // Regular peripheral hole
            const diameter = parseFloat($inputs.eq(0).val());
            const distance = parseFloat($inputs.eq(1).val());
            const angle = parseFloat($inputs.eq(2).val());
            
            // Find the correct hole index in the regenerated system
            // If we have central lumen, peripheral holes start at index 1
            const systemHoleIndex = system.includeCentralLumen ? lineIndex : lineIndex - (system.includeCentralLumen ? 0 : 1);
    ...
    // Final geometry and UI updates
    system.createCylinder();
    system.createHoleMarkers();
    system.createHoleUI(); // This will create the proper UI with central lumen as h0
}
