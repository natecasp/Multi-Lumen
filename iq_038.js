...
switchCamera() {
    ...
    if (cameraButton) {
        cameraButton.textContent = this.isPerspectiveView ? 'SWITCH TO TOP VIEW (SPACE)' : 'SWITCH TO 3D VIEW (SPACE)';
...
addCameraSwitchButton() {
    ...
    cameraButtonContainer.innerHTML = `
        <button id="camera-switch-btn" class="feet-tag" style="cursor:pointer; background:none;">SWITCH TO TOP VIEW (SPACE)</button>
...
togglePrintMode() {
    ...
    if (printButton) {
        printButton.textContent = this.isPrintMode ? 'EXIT PRINT MODE (TAB)' : 'PRINT MODE (TAB)';
