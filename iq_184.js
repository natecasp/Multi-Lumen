  ...
  updateCylinderGeometry() {
   ...
   autoExitPrintMode();
    ...
    if (isAltPressed) {
        ...
        // Apply to active hole only - be more careful about validation
        const hole = this.holes[this.activeHoleIndex];
        const { x, y } = this.polarToCartesian(hole.angle, newDistance);
        ...
        if (this.isValidPosition(x, y, hole.radius, this.activeHoleIndex)) {
            ...
            const oldDistance = hole.distance;
            const oldX = hole.x;
            const oldY = hole.y;
        ...
        // If position is not valid, don't update anything - this should prevent the overshoot
    } else {
...
updateSingleHole(holeIndex) {
    ...
    // IMPORTANT FIX: Ensure hole coordinates match the validated positions
    // This prevents the geometry from using invalid coordinates
    if (marker) {
        hole.x = marker.position.x;
        hole.y = marker.position.y;
        hole.distance = Math.sqrt(hole.x * hole.x + hole.y * hole.y);
    }
    
    // Now update geometry with validated coordinates
    this.updateCylinderGeometry();
    this.updateHoleUI(holeIndex);
...
isValidPosition(x, y, radius, holeIndex) {
    ...
    const maxDistance = this.cylinderRadius - spacingConstraint;
    if (distanceFromCenter > maxDistance) {
...
updateAllHoleDistanceLimits() {
  ...
  updateSingleHoleDistanceLimitsOnly(holeIndex) {
    ...
    const cylinderInnerEdge = spacingConstraint;
    const semicircleOuterEdge = this.semicircleRadius;
    const holeRadius = hole.radius;
