...
function setupCameraSwitching() {
    ...
    // Update all peripheral holes to the new distance
  ...
  findClosestValidRadius(x, y, targetRadius, holeIndex) {
    ...
    globalRenderer.domElement.addEventListener('mousemove', (event) => {
        ...
        if (isAltPressed && isControlPressed) {
            // Alt + Control: Set ALL holes to the same size (existing functionality)
            const currentMouseDistance = Math.sqrt(intersectPoint.x * intersectPoint.x + intersectPoint.y * intersectPoint.y);
            const maxAllowedRadius = this.cylinderRadius;
            ...
            this.holes.forEach((h, index) => {
                ...
                if (constrainedRadius !== h.radius) {
                    ...
                    if (Math.abs(h.x) < 0.001 && Math.abs(h.y) < 0.001) {
                        this.centralLumenRadius = constrainedRadius;
            ...
            // Shift: Change distance from center for ALL peripheral holes
            const newDistance = Math.sqrt(intersectPoint.x * intersectPoint.x + intersectPoint.y * intersectPoint.y);
            
            this.holes.forEach((h, index) => {
                if (h.distance > 0) { // Only update peripheral holes, skip central lumen
                    const { x, y } = this.polarToCartesian(h.angle, newDistance);
                    if (this.isValidPosition(x, y, h.radius, index)) {
                        h.distance = newDistance;
                        h.x = x;
                        h.y = y;
                    }
                }
            ...
            // Control: Change hole diameter (existing functionality)
            ...
            // Normal drag: Change angle around center (existing functionality)
            const isCentralHole = (Math.abs(hole.x) < 0.001 && Math.abs(hole.y) < 0.001);
...
function calculate() {
  if (checkMaterial() && checkOD() && checkLength() && checkQuantity() && checkCrescent()) {
...
function getCircularProfileData(lineItem) {
    ...
    // Add global distance control first if there are peripheral holes
    if (peripheralHole) {
        holeTML += `<div class="hole-layout">
            <p class="label quarter">Global Distance</p>
            <input id="l${lineItem}-global-distance" class="quote-input quarter" value="${globalDistance}" readonly></input>
...
function populateCircularProfile(lineNumber) {
    ...
    if (system instanceof CircularHolesSystem) {
        ...
        setTimeout(() => {
            populateCircularHoleDataFromLine($holeLayouts, system);
        }, 500);
    }
}

function populateCircularHoleDataFromLine($holeLayouts, system) {
    ...
    $holeLayouts.each(function(lineIndex) {
        ...
        if (label === 'Global Distance') {
            globalDistance = parseFloat($inputs.eq(0).val());
            console.log('Found global distance:', globalDistance);
        } else if (label === 'Central Lumen') {
            ...
            // Regular peripheral hole (now only has diameter and angle)
            const diameter = parseFloat($inputs.eq(0).val());
            ...
            if (system.includeCentralLumen) {
                // Skip the central lumen and global distance entries
                const adjustedIndex = lineIndex - 1; // Subtract 1 for global distance
                systemHoleIndex = adjustedIndex; // Central lumen is at index 0, peripherals start at 1
            } else {
                // Skip the global distance entry
                const adjustedIndex = lineIndex - 1; // Subtract 1 for global distance
           systemHoleIndex = adjustedIndex; // All holes are peripheral, start at 0
       }
