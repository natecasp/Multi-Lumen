...
createInterHoleSpacingDimensions() {
    ...
    if (isUniform) {
        ...
        if (peripheralHoles.length === 2) {
            // Only 2 holes: use holes 1 and 2 for spacing
            spacingHole1 = peripheralHoles[0];
            spacingHole2 = peripheralHoles[1];
        } else {
            // 3+ holes: use holes 2 and 3 for spacing
            spacingHole1 = peripheralHoles[1];
            spacingHole2 = peripheralHoles[2];
        }
        
        this.createLinearSpacingDimension(spacingHole1, spacingHole2, z, unit, holeCenterRadius);
        ...
        for (let i = 0; i < iterationCount; i++) {
            ...
            this.createLinearSpacingDimension(currentHole, nextHole, z, unit, holeCenterRadius);
        }
    }
}

// New helper method to create a single linear spacing dimension
createLinearSpacingDimension(currentHole, nextHole, z, unit, holeCenterRadius) {
