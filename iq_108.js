...
function updateAllInputValues(fromUnit, toUnit) {
    ...
    updateInputConstraints('#septum-thickness', toUnit, baseValues.minLength, baseValues.maxLength, 'length');
...
// Update input constraints (min, max, step)
function updateInputConstraints(selector, unit, minValue, maxValue, type) {
    const input = document.querySelector(selector);
    if (input) {
        input.min = minValue.toFixed(unit === 'mm' ? 2 : 3);
        input.max = maxValue.toFixed(unit === 'mm' ? 2 : 3);
...
getMinimumSeptumThickness() {
  const unit = $("#unit").val() || 'in';
    ...
    return outerDiameter <= getBaseValues(unit).defaultCylinderRadius*2 ? baseValues.minimumSeptum : baseValues.minimumSeptum * 1.6;
}
