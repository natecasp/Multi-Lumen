...
captureModelImage(width = 300, height = 300) {
    return new Promise((resolve) => {
        ...
        // Temporarily set both renderers to square dimensions
        this.renderer.setSize(width, height);
        if (this.css2dRenderer) {
            this.css2dRenderer.setSize(width, height);
        }
        
        // Set camera for square aspect ratio WITHOUT squishing
        this.setOptimalCameraForThumbnail();
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    ...
                    try {
                        // Draw WebGL content directly (no cropping since we set square dimensions)
                        finalCtx.drawImage(this.renderer.domElement, 0, 0, width, height);
                        console.log('Drew WebGL content');
                    } catch (error) {
                    ...
                    if (this.isPrintMode && this.css2dRenderer && 
                        this.css2dRenderer.domElement.style.display !== 'none') {
                        this.drawCss2dElementsImproved(finalCtx, width, height);
                    }
                ...
                }, 300); // Even longer wait for CSS2D
            }, 150);
        }, 100);
    });
}
    ...
    this.scene.traverse((object) => {
        if (object.isCSS2DObject && object.visible) {
            ...
            if (x >= 0 && x <= width && y >= 0 && y <= height) {
                ...
                if (text) {
                    // Set font properties (scale down for thumbnail)
                    const fontSize = 10; // Fixed size for thumbnails
...
setOptimalCameraForThumbnail() {
    ...
    this.holes.forEach(hole => {
        ...
        const extent = holeExtent + 0.02; // Add some padding
    ...
    if (this.isPrintMode) {
        const extensionRadius = this.cylinderRadius + 0.06; // Space for dimension lines
    ...
    // Set square orthographic camera (equal left/right and top/bottom)
