...
updateSliderValues(holeIndex) {
  ...
  captureModelImage(width = 300, height = 300) {
    return new Promise((resolve) => {
        ...
        // Set optimal camera position and zoom for thumbnail
        ...
        requestAnimationFrame(() => {
            ...
            const sourceSize = Math.min(rendererCanvas.width, rendererCanvas.height);
            const sourceX = (rendererCanvas.width - sourceSize) / 2;
            const sourceY = (rendererCanvas.height - sourceSize) / 2;
...
setOptimalCameraForThumbnail() {
    // Calculate the bounding box of all holes plus cylinder
    let minX = -this.cylinderRadius;
    let maxX = this.cylinderRadius;
    let minY = -this.cylinderRadius;
    let maxY = this.cylinderRadius;
    
    // Extend bounds to include dimension labels if they exist
    if (this.dimensionLabels.length > 0) {
        minX = Math.min(minX, -this.cylinderRadius * 1.5);
        maxX = Math.max(maxX, this.cylinderRadius * 1.5);
        minY = Math.min(minY, -this.cylinderRadius * 1.5);
        maxY = Math.max(maxY, this.cylinderRadius * 1.5);
    }
    ...
    // Set orthographic camera zoom to fit the content with 20% padding
    const padding = 1.2;
    const requiredFrustumSize = maxRange * padding;
    
    // Update orthographic camera
    const aspect = 1; // Square aspect ratio for thumbnail
    this.orthographicCamera.left = -requiredFrustumSize / 2;
    this.orthographicCamera.right = requiredFrustumSize / 2;
    this.orthographicCamera.top = requiredFrustumSize / 2;
    this.orthographicCamera.bottom = -requiredFrustumSize / 2;
