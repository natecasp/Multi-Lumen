...
const material = new THREE.MeshStandardMaterial({ 
    ...
    <div class="input-group">
        ...
        <input class="range-slider c-${hole.color}" type="range" min="0.015" max="${maxHoleDiameter.toFixed(3)}" value="${(hole.radius * 2).toFixed(4)}" step="0.001" id="hole-${index}-diameter-range" >
    ...
    `;
        ...
        <div class="input-group">
            <label>Distance:</label>
            <input type="range" class="range-slider c-${hole.color}" min="0" max="${(this.cylinderRadius * 0.8).toFixed(3)}" value="${hole.distance.toFixed(4)}" step="0.001" id="hole-${index}-distance-range">
...
$("#unit").on('change', function(e, previousUnit) {
    ...
    const oldUnit = previousUnit || (newUnit === 'mm' ? 'in' : 'mm'); // Default previous unit
    
    // Convert all dimensions
    convertAllDimensions(oldUnit, newUnit);
    
    // Update any system-specific limits that depend on units
    if (window.profileManager && window.profileManager.currentSystem) {
