...
snapAngleToIncrement(angle) {
 ...
 //// SEPTUM MEASUREMENT
 // Calculate distance between two holes minus their radii (septum thickness)
calculateHoleToHoleSeptum(hole1Index, hole2Index) {
    const hole1 = this.holes[hole1Index];
    const hole2 = this.holes[hole2Index];
    
    const dx = hole1.x - hole2.x;
    const dy = hole1.y - hole2.y;
    const centerDistance = Math.sqrt(dx * dx + dy * dy);
    
    return centerDistance - hole1.radius - hole2.radius;
}

// Calculate distance from hole edge to cylinder wall (septum thickness)
calculateHoleToCylinderSeptum(holeIndex) {
    const hole = this.holes[holeIndex];
    const distanceFromCenter = Math.sqrt(hole.x * hole.x + hole.y * hole.y);
    
    return this.cylinderRadius - distanceFromCenter - hole.radius;
}

// Get all septum thicknesses in the design
getAllSeptumThicknesses() {
    const septumData = [];
    
    // Get hole-to-hole septum thicknesses
    for (let i = 0; i < this.holes.length; i++) {
        for (let j = i + 1; j < this.holes.length; j++) {
            const thickness = this.calculateHoleToHoleSeptum(i, j);
            septumData.push({
                type: 'hole-to-hole',
                thickness: thickness,
                hole1Index: i,
                hole2Index: j,
                hole1Name: this.holes[i].name,
                hole2Name: this.holes[j].name
            });
        }
    }
    
    // Get hole-to-cylinder septum thicknesses
    // Skip central holes at (0,0) - they don't have meaningful septum to cylinder wall
    for (let i = 0; i < this.holes.length; i++) {
        const hole = this.holes[i];
        const isCentralHole = (Math.abs(hole.x) < 0.001 && Math.abs(hole.y) < 0.001);
        
        if (!isCentralHole) {
            const thickness = this.calculateHoleToCylinderSeptum(i);
            septumData.push({
                type: 'hole-to-cylinder',
                thickness: thickness,
                holeIndex: i,
                holeName: this.holes[i].name
            });
        }
    }
...
// Check if septum thicknesses vary by more than 20%
checkSeptumThicknessVariation() {
    const septumData = this.getAllSeptumThicknesses();
    const thicknesses = septumData.map(data => data.thickness);
    
    if (thicknesses.length === 0) return { hasWarning: false };
    
    const minThickness = Math.min(...thicknesses);
    const maxThickness = Math.max(...thicknesses);
    
    // Check if max is more than 20% greater than min
    const variationPercentage = ((maxThickness - minThickness) / minThickness) * 100;
    const hasWarning = variationPercentage > 20;
    
    if (hasWarning) {
        // Find which septums are causing the issue
        const minSeptum = septumData.find(data => data.thickness === minThickness);
        const maxSeptum = septumData.find(data => data.thickness === maxThickness);
        
        return {
            hasWarning: true,
            variationPercentage: variationPercentage.toFixed(1),
            minThickness: minThickness.toFixed(4),
            maxThickness: maxThickness.toFixed(4),
            minSeptum: minSeptum,
            maxSeptum: maxSeptum,
            allSeptumData: septumData
        };
    }
...
// Display septum warning in UI
displaySeptumWarning(warningData) {
    // Remove existing warning if present
    const existingWarning = document.getElementById('septum-warning');
    if (existingWarning) {
        existingWarning.remove();
    }
    
    if (!warningData.hasWarning) return;
    
    const warningDiv = document.createElement('div');
    warningDiv.id = 'septum-warning';
    ...
    if (warningData.minSeptum.type === 'hole-to-hole') {
        minDescription = `between ${warningData.minSeptum.hole1Name} and ${warningData.minSeptum.hole2Name}`;
    } else {
        minDescription = `between ${warningData.minSeptum.holeName} and cylinder wall`;
    }
    
    if (warningData.maxSeptum.type === 'hole-to-hole') {
        maxDescription = `between ${warningData.maxSeptum.hole1Name} and ${warningData.maxSeptum.hole2Name}`;
    } else {
        maxDescription = `between ${warningData.maxSeptum.holeName} and cylinder wall`;
    }
    
    warningDiv.innerHTML = `
        <strong>⚠️ Septum Thickness Warning</strong><br>
        Septum thicknesses vary by ${warningData.variationPercentage}% (>20% threshold)<br>
        <br>
        <strong>Thinnest:</strong> ${warningData.minThickness} ${minDescription}<br>
        <strong>Thickest:</strong> ${warningData.maxThickness} ${maxDescription}<br>
        <br>
        <small>Consider adjusting hole positions or sizes for more uniform septum thickness.</small>
    `;
    
    // Insert warning at the top of controls
    const controlsContainer = document.getElementById('hole-controls');
    controlsContainer.insertBefore(warningDiv, controlsContainer.firstChild);
}

// Call this method whenever holes are updated to check for warnings
updateSeptumWarnings() {
    const warningData = this.checkSeptumThicknessVariation();
    this.displaySeptumWarning(warningData);
}
 //// END SEPTUM MEASUREMENT
...
generateInitialHoles(count) {
    ...
    if (peripheralHoles > 0) {
        ...
        const placementRadius = Math.min(0.05, this.cylinderRadius - optimalRadius - this.minimumSeptum - 0.01);
        
        for (let i = 0; i < peripheralHoles; i++) {
            const angle = (i / peripheralHoles) * 360;
            const distance = Math.min(0.05, this.cylinderRadius - optimalRadius - this.minimumSeptum - 0.01);
...
setupRenderer() {
    ...
    this.renderer.setSize(sandbox.innerWidth, sandbox.innerHeight);
    ...
    // Add CSS2D renderer for dimensions
    this.css2dRenderer = new CSS2DRenderer();
    this.css2dRenderer.setSize(sandbox.innerWidth, sandbox.innerHeight);
...
setupCamera() {
    ...
    this.perspectiveCamera = new THREE.PerspectiveCamera(50, sandbox.innerWidth / sandbox.innerHeight, 0.001, 2);
    this.perspectiveCamera.position.set(0.2, 0.2, 0.3);
    this.scene.add(this.perspectiveCamera);
    
    // Create orthographic camera
    const aspect = sandbox.innerWidth / sandbox.innerHeight;
...
addCameraSwitchButton() {
    ...
    cameraButtonContainer.style.marginTop = '15px';
    cameraButtonContainer.innerHTML = `
        <button id="camera-switch-btn" style="
            ...
            width: 100%;
            margin-bottom: 10px;
        ">Switch to Top View</button>
        <button id="print-mode-btn" style="
            padding: 10px 20px; 
            background: #4CAF50; 
            color: white; 
            border: none; 
...
setupGlobalControls() {
    ...
    const cylinderDiameterValue = document.getElementById('cylinder-diameter-value');
    const cylinderWarning = document.getElementById('cylinder-warning');
    
    // Set initial values
    cylinderDiameterSlider.value = this.cylinderRadius * 2;
    cylinderDiameterInput.value = this.cylinderRadius * 2;
    cylinderDiameterValue.textContent = (this.cylinderRadius * 2).toFixed(3);
    ...
    cylinderDiameterSlider.addEventListener('input', (e) => {
        ...
        this.updateCylinderDiameter(newDiameter, cylinderDiameterInput, cylinderDiameterValue, cylinderWarning);
    });
...
updateCylinderDiameter(newDiameter, inputElement, valueDisplay, warningElement) {
    ...
    if (newRadius >= minAllowedRadius) {
        ...
        valueDisplay.textContent = newDiameter.toFixed(3);
        warningElement.style.display = 'none';
        
        this.updatePositionSliderRanges();
        this.updateCylinderGeometry();
        // Add septum warning check
        this.updateSeptumWarnings();
        this.render();
    } else {
        ...
        valueDisplay.textContent = minDiameter.toFixed(3);
    ...
    if (isAltPressed && isControlPressed) {
        ...
        if (!isCentralHole) {
            ...
            if (this.isValidPolarPosition(hole.angle, newDistance, hole.radius, holeIndex)) {
                ...
                // Note: updateHoleFromPolar already calls updateSeptumWarnings()
            }
        }
    } else {
        // Normal drag: Change angle around center
        const currentMouseAngle = Math.atan2(intersectPoint.y, intersectPoint.x) * 180 / Math.PI;
...
updateAllHoleMarkersAndUI() {
    this.holes.forEach((hole, index) => {
        ...
        const diameterSlider = document.getElementById(`diameter-${index}`);
        const diameterInput = document.getElementById(`diameter-input-${index}`);
        const diameterValue = document.getElementById(`diameter-value-${index}`);
        ...
        if (diameterValue) diameterValue.textContent = (hole.radius * 2).toFixed(4);
    });
...
updateHoleDiameterDirect(holeIndex, newRadius) {
    if (this.isValidPosition(this.holes[holeIndex].x, this.holes[holeIndex].y, newRadius, holeIndex)) {
        ...
        // Update UI sliders
        const diameterSlider = document.getElementById(`diameter-${holeIndex}`);
        const diameterInput = document.getElementById(`diameter-input-${holeIndex}`);
        const diameterValue = document.getElementById(`diameter-value-${holeIndex}`);
        
        if (diameterSlider) diameterSlider.value = (newRadius * 2).toFixed(4);
        if (diameterInput) diameterInput.value = (newRadius * 2).toFixed(4);
        if (diameterValue) diameterValue.textContent = (newRadius * 2).toFixed(4);
        // Add septum warning check
        this.updateSeptumWarnings();
        this.render();
        return true;
...
updatePositionSliderRanges() {
    ...
    this.holes.forEach((hole, index) => {
        const distanceSlider = document.getElementById(`distance-${index}`);
        const distanceInput = document.getElementById(`distance-input-${index}`);
...
createHoleControlsUI() {
    ...
    this.holes.forEach((hole, index) => {
        ...
        holeControl.className = 'div-block-264';
        ...
        let controlsHTML = `
          <div class="input-box">
            <div class="slidecontainer">
              <input class="range-slider" type="range" min="0.015" max="0.06" value="${(hole.radius * 2).toFixed(4)}" step="0.001" id="h${index}-diameter-range" >
            </div>
            <input class="text-field-3 slider w-input" type="number" id="h${index}-diameter" value="${(hole.radius * 2).toFixed(4)}" min="0.015" max="0.06" step="0.001">
            <span class="value-display"style="display:none;" id="h${index}-diameter-value">${(hole.radius * 2).toFixed(4)}</span>
          </div>
          `;
        ...
        if (!isCentralLumen) {
            ...
            <input class="text-field-3 slider w-input" type="number" id="h${index}-distance" min="0" max="${maxPosition.toFixed(3)}" value="${hole.distance.toFixed(4)}" step="0.001">
            <span class="value-display"style="display:none;" id="h${index}-distance-value">${hole.distance.toFixed(4)}</span>
          </div>
          ...
          <div class="input-box">
            ...
            <input class="text-field-3 slider w-input" type="number" id="h${index}-angle" min="0" max="360" value="${hole.angle}" step="1">
            <span class="value-display"style="display:none;" id="h${index}-angle-value">${Math.round(hole.angle)}</span>
          </div>`;
        }
...
setupHoleSliderListeners(index) {
    ...
    const diameterValue = document.getElementById(`h${index}-diameter-value`);
    ...
    diameterSlider.addEventListener('input', (e) => {
        ...
        this.updateHoleDiameter(index, newDiameter, diameterValue, warning);
    });
    ...
    if (!isCentralLumen) {
        ...
        const angleValue = document.getElementById(`h${index}-angle-value`);
        const distanceValue = document.getElementById(`h${index}-distance-value`);
        
        // Angle controls
        angleSlider.addEventListener('input', (e) => {
            const newAngle = parseFloat(e.target.value);
            angleInput.value = Math.round(newAngle);
            this.updateHoleAngle(index, newAngle, angleValue, warning);
        });
        ...
        distanceSlider.addEventListener('input', (e) => {
            ...
            this.updateHoleDistance(index, newDistance, distanceValue, warning);
        });
...
updateHoleDiameter(index, newDiameter, valueDisplay, warningElement) {
    const newRadius = newDiameter / 2;
    
    if (this.isValidPosition(this.holes[index].x, this.holes[index].y, newRadius, index)) {
        this.holes[index].radius = newRadius;
        this.recordValidPosition(index);
        valueDisplay.textContent = newDiameter.toFixed(4);
        this.updateHoleMarker(index);
        this.updateCylinderGeometry();
        warningElement.style.display = 'none';
        // Add septum warning check
        ...
        this.showWarning(warningElement);
    }
}

updateHoleAngle(index, newAngle, valueDisplay, warningElement) {
    // Snap angle to 1-degree increments
    const snappedAngle = this.snapAngleToIncrement(newAngle);
    
    if (this.isValidPolarPosition(snappedAngle, this.holes[index].distance, this.holes[index].radius, index)) {
        this.holes[index].angle = snappedAngle;
        const { x, y } = this.polarToCartesian(snappedAngle, this.holes[index].distance);
        this.holes[index].x = x;
        this.holes[index].y = y;
        this.recordValidPosition(index);
        valueDisplay.textContent = Math.round(snappedAngle) + '°';
        
        // Update the slider to reflect the snapped value
        const angleSlider = document.getElementById(`angle-${index}`);
        const angleInput = document.getElementById(`angle-input-${index}`);
        if (angleSlider) angleSlider.value = snappedAngle;
        if (angleInput) angleInput.value = Math.round(snappedAngle);
...
updateHoleDistance(index, newDistance, valueDisplay, warningElement) {
    if (this.isValidPolarPosition(this.holes[index].angle, newDistance, this.holes[index].radius, index)) {
        this.holes[index].distance = newDistance;
        const { x, y } = this.polarToCartesian(this.holes[index].angle, newDistance);
        this.holes[index].x = x;
        this.holes[index].y = y;
        this.recordValidPosition(index);
        valueDisplay.textContent = newDistance.toFixed(4);
        this.updateHoleMarkerPosition(index);
        this.updateCylinderGeometry();
        warningElement.style.display = 'none';
        // Add septum warning check
...
updateSliderValues(holeIndex) {
    ...
    const angleSlider = document.getElementById(`angle-${holeIndex}`);
    const angleInput = document.getElementById(`angle-input-${holeIndex}`);
    const angleValueDisplay = document.getElementById(`angle-value-${holeIndex}`);
    
    const distanceSlider = document.getElementById(`distance-${holeIndex}`);
    const distanceInput = document.getElementById(`distance-input-${holeIndex}`);
    const distanceValueDisplay = document.getElementById(`distance-value-${holeIndex}`);

    // Update angle controls
    if (angleSlider && angleInput && angleValueDisplay) {
        angleSlider.value = hole.angle;
        angleInput.value = Math.round(hole.angle);
        angleValueDisplay.textContent = Math.round(hole.angle) + '°';
    }
    
    // Update distance controls
    if (distanceSlider && distanceInput && distanceValueDisplay) {
        distanceSlider.value = hole.distance.toFixed(4);
        distanceInput.value = hole.distance.toFixed(4);
        distanceValueDisplay.textContent = hole.distance.toFixed(4);
    }
}
...
for(var i = 0; i < holes.length; i++){
  ...
  var holeTitle = $("#"+holes[i].id).find('.hole-title').text();
  var holeNum = i + 1;
  var diameter = $("#"+holes[i].id).find('.diameter').val();
  var distance = $("#"+holes[i].id).find('.distance').val();;
  var angle = $("#"+holes[i].id).find('.angle').val();
  
  holeTML += `<div class="hole-layout">
    <p class="label">`+holeTitle+`</p>
    <input class="quote-input" id="l`+lineItem+`-h`+holeNum+`-diameter" value="`+diameter+`" readonly></input>
    <input class="quote-input" id="l`+lineItem+`-h`+holeNum+`-distance" value="`+distance+`" readonly></input>
    <input class="quote-input" id="l`+lineItem+`-h`+holeNum+`-angle" value="`+angle+`" readonly></input>
  </div>`
}
