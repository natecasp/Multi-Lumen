...
captureModelImage(captureWidth = 600, captureHeight = 600) {
    return new Promise((resolve) => {
        ...
        // Temporarily set both renderers to high resolution square dimensions
        this.renderer.setSize(captureWidth, captureHeight);
        if (this.css2dRenderer) {
            this.css2dRenderer.setSize(captureWidth, captureHeight);
        }
        
        // Set camera for square aspect ratio with more zoom out
        this.setOptimalCameraForThumbnail(true); // Pass true for more zoom out
        
        // Force multiple renders with delays to ensure everything draws
        this.render();
        
        // Wait for CSS2D elements to position themselves
        setTimeout(() => {
            this.render();
            ...
            setTimeout(() => {
                ...
                // Wait even longer for CSS2D positioning
                setTimeout(() => {
                    // Final render
                    this.render();
                    
                    // Create final composite canvas at high resolution
                    ...
                    try {
                        // Draw WebGL content directly
                        finalCtx.drawImage(this.renderer.domElement, 0, 0, captureWidth, captureHeight);
                        console.log('Drew WebGL content at', captureWidth, 'x', captureHeight);
                    } catch (error) {
                        console.error('Error drawing WebGL:', error);
                    }
                    
                    // Manually draw CSS2D elements with better positioning
                    if (this.isPrintMode && this.css2dRenderer && 
                        this.css2dRenderer.domElement.style.display !== 'none') {
                        this.drawCss2dElementsImproved(finalCtx, captureWidth, captureHeight);
                    }
                    
                    // Get final image
                    const imageDataUrl = finalCanvas.toDataURL('image/png');
                    
                    // Restore original renderer sizes
...
drawCss2dElementsImproved(ctx, width, height) {
    // Calculate scale factor based on resolution
    const scaleFactor = width / 300; // Base scale on 300px reference
        ...
        if (object.isCSS2DObject && object.visible) {
            ...
            if (x >= 0 && x <= width && y >= 0 && y <= height) {
                ...
                if (text) {
                    ...
                    // Draw background if the element has one
                    ...
                    lines.forEach((line, index) => {
                        if (line.trim()) {
                            const lineY = y + (index - (lines.length - 1) / 2) * (fontSize + 3 * scaleFactor);
    ...
    console.log('Drew CSS2D elements at scale:', scaleFactor);
}

// Add this method to set optimal camera position for thumbnails
setOptimalCameraForThumbnail(extraZoomOut = false) {
    // Calculate bounds including all content
    let minX = -this.cylinderRadius;
    let maxX = this.cylinderRadius;
    let minY = -this.cylinderRadius;
    let maxY = this.cylinderRadius;
    
    // Check all holes to extend bounds
    this.holes.forEach(hole => {
        minX = Math.min(minX, hole.x - hole.radius - 0.02);
        maxX = Math.max(maxX, hole.x + hole.radius + 0.02);
        minY = Math.min(minY, hole.y - hole.radius - 0.02);
        maxY = Math.max(maxY, hole.y + hole.radius + 0.02);
    });
    
    // Extend bounds for dimension labels (they extend beyond cylinder)
    if (this.isPrintMode) {
        const extensionRadius = this.cylinderRadius + 0.08; // Increased from 0.06
        minX = Math.min(minX, -extensionRadius);
        maxX = Math.max(maxX, extensionRadius);
        minY = Math.min(minY, -extensionRadius);
        maxY = Math.max(maxY, extensionRadius);
    }
    
    // Calculate the range and make it square
    const rangeX = maxX - minX;
    const rangeY = maxY - minY;
    const maxRange = Math.max(rangeX, rangeY);
    
    // Add more padding - increased from 1.3 to 1.5, or 1.7 if extra zoom out
    const paddingMultiplier = extraZoomOut ? 1.7 : 1.5;
    const frustumSize = maxRange * paddingMultiplier;
    
    // Set square orthographic camera
