...
const BASE_VALUES_INCHES = {
    ...
    cameraMaxDistance: 1,           // Camera maximum distance
...
function updateCameraConstraints(unit) {
    ...
    // Calculate scale factor for the unit
    const scaleFactor = unit === 'mm' ? UNIT_CONVERSIONS.INCH_TO_MM : 1;
    ...
    // Update orthographic camera frustum - this is the key fix
    if (globalOrthographicCamera) {
        const frustumSize = baseValues.cameraFrustumSize;
        ...
        // CRITICAL: Update the camera position for the new scale
        // The camera needs to be positioned at the right distance for the new unit scale
        const currentDistance = Math.sqrt(
            globalOrthographicCamera.position.x ** 2 + 
            globalOrthographicCamera.position.y ** 2 + 
            globalOrthographicCamera.position.z ** 2
        );
        
        // Set appropriate distance for the unit
        const targetDistance = unit === 'mm' ? 7.62 : 0.3; // 7.62mm = 0.3 inches
        
        if (currentDistance > 0) {
            // Normalize and scale to target distance
            const scale = targetDistance / currentDistance;
            globalOrthographicCamera.position.multiplyScalar(scale);
        } else {
            // Default position if camera is at origin
            globalOrthographicCamera.position.set(0, 0, targetDistance);
        }
        
        globalOrthographicCamera.updateProjectionMatrix();
    }
    
    // Update perspective camera position as well
    if (globalPerspectiveCamera) {
        const currentDistance = Math.sqrt(
            globalPerspectiveCamera.position.x ** 2 + 
            globalPerspectiveCamera.position.y ** 2 + 
            globalPerspectiveCamera.position.z ** 2
        );
        
        const targetDistance = unit === 'mm' ? 7.62 : 0.3; // Same as orthographic
        
        if (currentDistance > 0) {
            const scale = targetDistance / currentDistance;
            globalPerspectiveCamera.position.multiplyScalar(scale);
        } else {
            globalPerspectiveCamera.position.set(targetDistance * 0.7, targetDistance * 0.7, targetDistance * 0.7);
        }
        
        globalPerspectiveCamera.updateProjectionMatrix();
    }
    
    // Update the controls target to remain at origin
    if (globalControls) {
    ...
    if (globalRenderer && globalScene && globalCurrentCamera) {
        ...
        // Also update CSS2D renderer if it exists
        if (css2dRenderer && css2dRenderer.domElement.style.display !== 'none') {
            css2dRenderer.render(globalScene, globalCurrentCamera);
        }
    }
    
    console.log(`Camera updated for ${unit}: frustum=${baseValues.cameraFrustumSize}, distance=${unit === 'mm' ? 7.62 : 0.3}`);
}
...
function ensureCameraPositionForUnit() {
    ...
    if (typeof system.setConsistentCameraForCapture === 'function') {
        ...
        const systemRadius = system.cylinderRadius || 0.1;
        const padding = 1.2;
...
globalScene.add(backRightLight);
    ...
    const frustumSize = getBaseValues(unit).cameraFrustumSize;
...
createHoleDimensionLines(hole, index) {
  const unit = $("#unit").val() || 'in';
    const z = this.cylinderHeight / 2 + 0.002;
    const extensionLength = getBaseValues(unit).minimumSeptum * 6;
...
setupCrescentInteraction() {
  const unit = $("#unit").val() || 'in';
    ...
    const sensitivity = getBaseValues(unit).defaultCylinderRadius;
...
createCrescentDimensions(z, geometry) {
    ...
    // Add tick marks
    const tickLength = getBaseValues(unit).minimumSeptum;
...
function checkLength() {
  const unit = $("#unit").val() || 'in';
  if(Number($("#length-2").val()) >= getBaseValues(unit).minLength && Number($("#length-2").val()) <= getBaseValues(unit).maxLength) {
    return true;
  } else {
    return false;
  }
