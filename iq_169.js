...
function checkSurfaceArea() {
  if(window.profileManager.currentSystem instanceof CircularHolesSystem) {
    ...
    var minRatio = Number($("#min-ratio").val());
    var maxRatio = Number($("#max-ratio").val());
    $("#ratio-reader").text(ratio);
...
function checkAndUpdateHoleDistribution() {
  if(window.profileManager.currentSystem instanceof CircularHolesSystem) {
    ...
    // OFFSET: Start at half a slice angle so holes are centered in segments
    const sliceOffset = sliceAngle / 2;
    
    console.log(`Checking distribution: ${peripheralHoles.length} holes, ${sliceAngle}° per slice, offset: ${sliceOffset}°`);
    
    // Check each slice
    for (let sliceIndex = 0; sliceIndex < peripheralHoles.length; sliceIndex++) {
      // Apply offset so slice boundaries don't align with expected hole positions
      const sliceStartAngle = (sliceIndex * sliceAngle + sliceOffset) % 360;
      const sliceEndAngle = ((sliceIndex + 1) * sliceAngle + sliceOffset) % 360;
      
      let sliceHasHole = false;
      
      // Check if any hole CENTER is within this slice
      peripheralHoles.forEach(hole => {
        const holeAngle = hole.angle; // degrees (0-360)
        
        // Check if the hole center angle is within the current slice
        if (angleWithinSlice(holeAngle, sliceStartAngle, sliceEndAngle)) {
          sliceHasHole = true;
        }
      });
...
function highlightEmptySlice(startAngleDeg, endAngleDeg, system) {
  ...
  const thetaLength = endAngleRad - startAngleRad + Math.PI * 2;
  }
