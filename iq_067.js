...
const material = new THREE.MeshStandardMaterial({ 
    ...
    <div class="input-box">
        ...
        <input class="text-field-3 slider w-input diameter" type="number" id="hole-${index}-diameter" value="${(hole.radius * 2).toFixed(4)}" min="0.015" max="${maxHoleDiameter.toFixed(3)}" step="0.001">
    ...
    `;
        ...
        <div class="input-group">
            ...
            <input type="range" min="0" max="${(this.cylinderRadius * 0.8).toFixed(3)}" value="${hole.distance.toFixed(4)}" step="0.001" id="hole-${index}-distance-range">
        ...
        <div class="input-group">
            <label>Angle:</label>
            <input type="range" min="0" max="360" value="${Math.round(hole.angle)}" step="1" id="hole-${index}-angle-range">
    ...
    instructions.style.cssText = 'margin-top: 10px; padding: 8px; background: rgba(255,255,255,0.1); border-radius: 3px; font-size: 11px;';
    instructions.innerHTML = `
        <strong>Interactive Controls:</strong><br>
        • <strong>Drag hole:</strong> Change angle<br>
        • <strong>Shift + Drag:</strong> Change distance<br>
        • <strong>Ctrl + Drag:</strong> Change diameter<br>
        • <strong>Alt + Ctrl:</strong> Same size for all<br>
        • <strong>Alt + Shift:</strong> Same distance for all
    `;
...
findMinimumDiameterForSliceCount(sliceCount) {
    ...
    container.innerHTML = `
        ...
        <div class="input-group">
            <label>Septum:</label>
            <input type="range" id="septum-thickness" min="0.005" max="0.050" value="${this.septumThickness.toFixed(3)}" step="0.001">
    ...
    div.innerHTML = `
        ...
        <div class="input-group">
            <input type="range" id="slice-${i}" min="45" max="315" value="${Math.round(this.sliceAngles[i] * 180 / Math.PI)}" step="1">
...
class SmileSystem {
   constructor() {
    ...
    this.holes = [];
    this.holeColors = [0x00ff00, 0x0088ff, 0xff4444, 0xffaa00];
    ...
    this.holes.forEach((hole, index) => {
        ...
        div.innerHTML = `
            ...
            <div class="input-group">
                ...
                <input type="range" id="smile-hole-${index}-diameter-range" min="0.005" max="${maxHoleDiameter.toFixed(3)}" value="${(hole.radius * 2).toFixed(4)}" step="0.001">
            ...
            <div class="input-group">
                <label>Angle:</label>
                <input type="range" id="smile-hole-${index}-angle-range" min="${minAngle}" max="${maxAngle}" value="${Math.round(useAngle)}" step="1">
                <input type="number" id="smile-hole-${index}-angle" min="${minAngle}" max="${maxAngle}" value="${Math.round(useAngle)}" step="1">
            </div>
            <div class="input-group">
                <label>Distance:</label>
                <input type="range" id="smile-hole-${index}-distance-range" min="${minDistance.toFixed(3)}" max="${maxDistance.toFixed(3)}" value="${useDistance.toFixed(4)}" step="0.001">
...
createSmileHoleUI() {
    ...
    this.holes.forEach((hole, index) => {
        ...
        div.innerHTML = `
            ...
            <div class="input-group">
                <label>Diameter:</label>
                <input type="range" id="smile-hole-${index}-diameter-range" min="0.005" max="0.06" value="${(hole.radius * 2).toFixed(4)}" step="0.001">
                <input type="number" id="smile-hole-${index}-diameter" min="0.005" max="0.06" value="${(hole.radius * 2).toFixed(4)}" step="0.001">
            </div>
            <div class="input-group">
                <label>Angle:</label>
                <input type="range" id="smile-hole-${index}-angle-range" min="0" max="360" value="${Math.round(hole.angle)}" step="1">
