...
function createOuterDiameterDimension(system, z) {
    ...
    // Position the dimension line at the bottom-right (315 degrees) to avoid conflicts
    const angle = -Math.PI / 4; // -45 degrees (315 degrees)
    const startX = Math.cos(angle) * outerRadius;
    const startY = Math.sin(angle) * outerRadius;
    const extensionDistance = outerRadius + 0.05;
    const endX = Math.cos(angle) * extensionDistance;
    const endY = Math.sin(angle) * extensionDistance;
    ...
    const crossLength = outerRadius * 0.05; // Small cross mark
    ...
    labelDiv.style.cssText = `
        ...
        white-space: nowrap;
    `;
    labelDiv.innerHTML = `<div><strong>Outer Diameter</strong></div><div>⌀ ${outerDiameter}m</div>`;
