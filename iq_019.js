...
setupGlobalControls() {
    ...
    cylinderDiameterSlider.addEventListener('input', (e) => {
        ...
        this.updateCylinderDiameter(newDiameter, cylinderDiameterInput, cylinderWarning);
    });
...
updateCylinderDiameter(newDiameter, inputElement, warningElement) {
...
updateAllHoleMarkersAndUI() {
    this.holes.forEach((hole, index) => {
        ...
        const cached = this.cachedDOMElements[index];
        if (cached) {
            if (cached.diameterSlider) cached.diameterSlider.value = (hole.radius * 2).toFixed(3);
            if (cached.diameterInput) cached.diameterInput.value = (hole.radius * 2).toFixed(3);
...
updateHoleDiameterDirect(holeIndex, newRadius) {
    if (this.isValidPosition(this.holes[holeIndex].x, this.holes[holeIndex].y, newRadius, holeIndex)) {
       ...
       const cached = this.cachedDOMElements[holeIndex];
        if (cached) {
            if (cached.diameterSlider) cached.diameterSlider.value = (hole.radius * 2).toFixed(3);
            if (cached.diameterInput) cached.diameterInput.value = (hole.radius * 2).toFixed(3);
...
updatePositionSliderRanges() {
    ...
    this.holes.forEach((hole, index) => {
      const cached = this.cachedDOMElements[index];
        if (cached) {
            if (cached.distanceSlider) cached.distanceSlider.max = maxDistance.toFixed(3);
            if (cached.distanceInput) cached.distanceInput.max = maxDistance.toFixed(3);
...
createHoleControlsUI() {
    ...
    this.holes.forEach((hole, index) => {
        ...
        this.cachedDOMElements[index] = {
        diameterSlider: document.getElementById(`h${index}-diameter-range`),
        diameterInput: document.getElementById(`h${index}-diameter`),
          distanceSlider: document.getElementById(`h${index}-distance-range`),
        distanceInput: document.getElementById(`h${index}-distance`),
          angleSlider: document.getElementById(`h${index}-angle-range`),
        angleInput: document.getElementById(`h${index}-angle`),
...
setupHoleSliderListeners(index) {
   ...
   const cached = this.cachedDOMElements[index];
    ...
    cached.diameterSlider.addEventListener('input', (e) => {
        const newDiameter = parseFloat(e.target.value);
        cached.diameterInput.value = newDiameter.toFixed(4);
        this.updateHoleDiameter(index, newDiameter, warning);
    });
    
    cached.diameterInput.addEventListener('input', (e) => {
        const newDiameter = parseFloat(e.target.value);
        if (!isNaN(newDiameter)) {
            cached.diameterSlider.value = newDiameter;
            this.updateHoleDiameter(index, newDiameter, warning);
        }
    });
    ...
    if (!isCentralLumen) {
        ...
        // Angle controls
        cached.angleSlider.addEventListener('input', (e) => {
            const newAngle = parseFloat(e.target.value);
            cached.angleInput.value = Math.round(newAngle);
            this.updateHoleAngle(index, newAngle, warning);
        });
        
        cached.angleInput.addEventListener('input', (e) => {
            const newAngle = parseFloat(e.target.value);
            if (!isNaN(newAngle)) {
                cached.angleSlider.value = newAngle;
                this.updateHoleAngle(index, newAngle, warning);
            }
        });
        
        // Distance controls
        cached.distanceSlider.addEventListener('input', (e) => {
            const newDistance = parseFloat(e.target.value);
            cached.distanceInput.value = newDistance.toFixed(4);
            this.updateHoleDistance(index, newDistance, warning);
        });
        
        cached.distanceInput.addEventListener('input', (e) => {
            const newDistance = parseFloat(e.target.value);
            if (!isNaN(newDistance)) {
                cached.distanceSlider.value = newDistance;
                this.updateHoleDistance(index, newDistance, warning);
            }
        });
    }//end cached
...
updateHoleDiameter(index, newDiameter, warningElement) {
    const newRadius = newDiameter / 2;
...
updateHoleAngle(index, newAngle, warningElement) {
    // Snap angle to 1-degree increments
    ...
    if (this.isValidPolarPosition(snappedAngle, this.holes[index].distance, this.holes[index].radius, index)) {
        ...
        if (cached.angleSlider) cached.angleSlider.value = snappedAngle;
        if (cached.angleInput) cached.angleInput.value = Math.round(snappedAngle);
...
updateHoleDistance(index, newDistance, warningElement) {
...
updateSliderValues(holeIndex) {
    ...
    if (cached.angleSlider && cached.angleInput) {
        cached.angleSlider.value = hole.angle;
        cached.angleInput.value = Math.round(hole.angle);
      
    }
    
    // Update distance controls
    if (cached.distanceSlider && cached.distanceInput) {
        cached.distanceSlider.value = hole.distance.toFixed(3);
        cached.distanceInput.value = hole.distance.toFixed(3);
        
    }
    if (cached.diameterSlider && cached.diameterInput) {
        cached.diameterSlider.value = (hole.radius*2).toFixed(3);
        cached.diameterInput.value = (hole.radius*2).toFixed(3);
